...
void create_watersuppress2() {
  ...
  if (!strcmp(wss,"vapor")) {
    ...
    init_rf(&ws_rf,wspat,pws,wsflipf*1.86*flipws,rof1,rof2);
  ...
  /***********************************************************/
    ...
    if (!strcmp(wss,"wet")) {
	 ...
	 and adjust the rest down through the fine power */
      ...
      init_rf(&ws_rf,wspat,pws,wsflipf*(161.0/69.3)*flipws,rof1,rof2);
      calc_rf(&ws_rf,"wstpwr","wstpwrf");
    ...
    /***********************************************************/
      ...
      init_rf(&ws_rf,wspat,pws,wsflipf*1.0*flipws,rof1,rof2);
      calc_rf(&ws_rf,"wstpwr","wstpwrf");
      if (!strcmp(wss,"dry")) {
        nwsp = 3; /* 3 pulses */
