...
/* het2dj.c - heteronuclear J-resolved experiment; absolute value mode
   ...
   Parameters:
       ...
       tpwr = power level for transmitter pulses
         pp = 90 degree decoupler pulse  (1H)
      pplvl = power level for decoupler pulses (if amptype='_a')
   ...
   revised   s. farmer    2  august 1988 */
...
/* CHECK CONDITIONS */
   if (dm[B] == 'y')
   {
      printf("DM must be set to 'n' for the second status.\n");
      psg_abort(1);
   }
   if (dm[C] == 'n')
   {
