...
static float alpha = 0.136;	/* Time constant for [CA-plasma]-signal decay */
		...
		float *y)	/* Function values OUT */
{
    int i;
    float offset;
    float recip;		/* Stores reciprocal of (1 - Hematocrit) */ 
    float decay;		/* Temporary storage for exp(-alpha*t) */ 
    float plasma_sig;		/* Signal from plasma CA */
    float ees_sig;		/* Signal from extra-vascular,-cellular space */
