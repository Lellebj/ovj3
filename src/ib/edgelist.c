...
*  -----------								*
*									*
*  This file contains routines to create edges of a polygon.		*
*  (Some of its routines may be used by oval).				*
*									*
*************************************************************************/
#include <math.h>
#include "stderr.h"
...
#ifdef DEBUG_BETA
static void debug_edge(Edgelist **, int, int, Gpoint *, int);
	...
	int offset)		// offset of 'y' position value
{
   ...
   for (i=0; i<ngpnts-1; i++)
   {
      ...
      {
         build_line_points(pntlist, gpnt[i].x, gpnt[i].y, 
			   gpnt[i+1].x, gpnt[i+1].y);
   ...
   {
      build_line_points(pntlist, gpnt[ngpnts-1].x, gpnt[ngpnts-1].y,
			gpnt[0].x, gpnt[0].y);
      sort_edge_list(yedge, offset, pntlist);
   }
...

{
    ...
    Flag done;			  // done or not done

    LpointIterator plist(lpnt);
    Lpoint *ptptr;		// pointer to the current edge point

    while (ptptr=++plist){
	xptr = new Edgelist(ptptr->x);
	if (yedge[ptptr->y - offset] == NULL){
	    yedge[ptptr->y - offset] = xptr;
	}else{
	    // Sort x_edge (inside the list) in increasing order
	    for (prev_x2ptr = x2ptr = yedge[ptptr->y - offset], done = FALSE;
		 x2ptr && (!done);
	...
	} // end of else (yedge[ptptr->y - offset] == NULL)
    } // end of while (..)
}
			    ...
			    short px2, short py2) // End point of line
{
    ...
    lpnt->Push(new Lpoint(px1, py1));

    // Store the rest of the pixels
    endy = py2;
    yinc = endy > py1 ? 1 : -1;
    for (rx = px1, y = py1; y != endy; ){
	rx += dx;
	y += yinc;
	lpnt->Push(new Lpoint((int)(rx + 0.5), y));
    }
}
	...
	int num)		// number of vertices
{
   ...
   if (!(((gpnt[0].y >= gpnt[num-1].y) && (gpnt[0].y >= gpnt[1].y)) ||
	((gpnt[0].y <= gpnt[num-1].y) && (gpnt[0].y <= gpnt[1].y))) )
      remove_one_edge(yedge[gpnt[0].y - offset], gpnt[0].x);
   ...
   for (i=1; i<num-1; i++)
   {
      if (!(((gpnt[i].y >= gpnt[i-1].y) && (gpnt[i].y >= gpnt[i+1].y)) ||
	   ((gpnt[i].y <= gpnt[i-1].y) && (gpnt[i].y <= gpnt[i+1].y))) )
         remove_one_edge(yedge[gpnt[i].y - offset], gpnt[i].x);
   }

   // LAST item
   if (!(((gpnt[num-1].y >= gpnt[0].y) && (gpnt[num-1].y >= gpnt[num-2].y)) ||
	((gpnt[num-1].y <= gpnt[0].y) && (gpnt[num-1].y <= gpnt[num-2].y))) )
      remove_one_edge(yedge[gpnt[num-1].y - offset], gpnt[num-1].x);
}
	...
	int ngpnts)		// number of vertices
{
	...
	while (v < last_v && (nny = gpnt[v+1].y) == y){
	    v++;
	}
	...
	if (prev_v != v && ((ppy - y) * (nny - y)) < 0){
	    ...
	    {
		small_x = gpnt[prev_v].x;
		large_x = gpnt[v].x;
	    }
	    else
	    {
		small_x = gpnt[v].x;
		large_x = gpnt[prev_v].x;
	    }
	    ...
	    if (!xptr){
		STDERR_2("No x_edge found at x=%d, y=%d",
			 small_x, gpnt[v].y);
	    }
	}
    }
