...
int main(argc, argv)
    ...
    int ns, nd, nsq, ncyc,celem;
    int ni, ni2, ni3, nimax, ni2max, ni3max, xdim;
    int *sch, nix[6];
    float f1d, f1z;
    double tm;

    void usage(), fix_pars();
    int ncycs[3]={1,1,1}; // complex=1,hypercomplex=2
    double csd1 = 0.0, csd2 = 0.0, csd3 = 0.0;
    ...
    nd = 1;
    j = getval_s(src, "array", str);

    if (getval(src, "CSdensity", &csd1) > 0) {
        if (getval_i(src, "ni", &i) > 0) {
            if(i>1 && csd1>0 && csd1<=100){
                nimax = i;
                nd *= i;
                ni = i * csd1 / 100.0;
                if(strcmp(str, "phase") == 0) // test if hypercomplex data in file
                    ncycs[0]=2;
            }
        }
        if (getval_i(src, "ni2", &i) > 0 && getval(src, "CSdensity2", &csd2)>0) {
            if(i>1 && csd2>0 && csd2<=100){
                ni2max = i;
                nd *= i;
                ni2 = ni2max * csd2 / 100.0;
                if((strcmp(str, "phase,phase2") == 0) || (strcmp(str, "phase2,phase") == 0))
                    ncycs[1]=2;
            }
        }
        if (getval_i(src, "ni3", &i) > 0 && getval(src, "CSdensity3", &csd3)>0) {
            if(i>1 && csd3>0 && csd3<=100){
                ni3max = i;
                nd *= i;
                ni3 = ni3max * csd3 / 100.0;
                // TODO: need parameter to test if hypercomplex or abs value in 4d
                //       - for now just assume data is hypercomplex
                    ncycs[2]=2;
            }
        }
    } else if (getval_i(src, "nimax", &nimax) > 0){
        getval_i(src, "ni", &ni);
        getval_i(src, "ni2", &ni2);
        getval_i(src, "ni3", &ni3);
        getval_i(src, "nimax", &nimax);
        getval_i(src, "ni2max", &ni2max);
        getval_i(src, "ni3max", &ni3max);
    }
    else
        vn_error("inflateND: neither CSdensity nor nimax defined");
    ...
    if ((sch == NULL )|| (i == 0) || (nsp !=i ))vn_error("sch2fid: failed to read the sampling schedule");
