...
/*	where 	tof=tof1,tof2						*/
/*		pw=0,pw,pw,pw...					*/
/*		d2=val1,val1,val2,val3,val4...				*/
/* The first arrayed acquisition for tof1 and tof2 is a noise acquire	*/
/* and that is why pw=0.						*/
/*----------------------------------------------------------------------*/
/* IMPORTANT:								*/
/* 	For beccphase n_cm,delf1,delf2 are not used.  They may be set	*/
/*	to zero and values that are calculated from them: G,a		*/
/*	are set to zero.						*/
/*----------------------------------------------------------------------*/
...
{  
	 ...
	 case 'a': 	
			...
			if (res != 1)
			   my_ave = 1;
                        break;
	 default:
			printf("beccphase: Invalid Arg = %s\n",tpntr);
			exit(1);
			break;
       }
     }
   ...
   if (mode > 0)
   {
	...
	outputdata(b0path,fbuff,nblks,&eccvals,&noise_val);
   }
exit(0);
}
...
{ int file_d_in,tot,bsize,i,npoints;
    ...
    if (header0.nblocks != nblks)
    {
	printf("beccphase: nblks = %d   header0.nblocks %d\n",
			nblks,header0.nblocks);
	exit(1);
    }
    if (header0.nblocks > MAX_BLKS)
    {
	printf("beccphase: header0.nblocks %d. Max allowed is %d\n",
...
int nblks;
{
   ...
   for (f1cntr=1; f1cntr<nblks; f1cntr++)
   {
	...
	if ((f1cntr+1) < nblks)
	   nextarray_time = eccvals->Tval[f1cntr+1];
	else
	...
	{
	   if ( *noise_val <= *(f1ptr+1) )
	   {
	      /* output zero order eddy current values */
