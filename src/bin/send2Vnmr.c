...
int find_VnmrMaster(int *masterpid, int maxpids)
{
...
int main(int argc, char *argv[])
{
    ...
    int i;
    int stat;
    int master_pid[MAX_MASTERS], argtype;
    int master_cnt = 0;
    unsigned long vpid = 0L;
...
*/
    ...
    */
    ...
    switch(argtype)
    {
      ...
      case VALL:
		if ((master_cnt = find_VnmrMaster(&master_pid[0],MAX_MASTERS)) == 0)
		{
      ...
      case VFILE:
		...
		{
                   if (debug)
		      printf("send2Vnmr: Vnmr access file error\n");
		   exit(1);
		}
		if ( (cptr = fgets(tmpstr, 120, accessfile )) == NULL)
		{
                   if (debug)
		      perror("send2Vnmr: Vnmr file read error\n");
		   exit(1);
		}
	        chrptr = strtok(tmpstr,"\n");
		strncpy(addr,chrptr,MAX_ADDRLEN-1);
		addr[MAX_ADDRLEN-1] = '\0';
/*
		chrptr = strtok(tmpstr," \n");
...
*/
    ...
    for(i=0; i< master_cnt; i++)
    {
       ...
       {
          if (debug)
             printf("send2Vnmr: Could not send message to Vnmr: '%s'\n",addr);
          exit(1);
       }
       if (debug)
