	...
	char **argv;
{
	 ...
	 * Log startup
	 */
	if (debug) report(LOG_INFO, "version %s.%d", VERSION, PATCHLEVEL);
	...
	for (;;) {
		...
		switch (bp->bp_op) {
		case BOOTREQUEST:
            		if (debug) report(LOG_INFO,"BOOT: BOOTREQUEST");
			handle_request();
			break;
		case BOOTREPLY:
            		if (debug) report(LOG_INFO,"BOOT: BOOTREPLY");
			handle_reply();
			break;
		}
	}
...
ignoring request for server %s from client at %s address %s",
        ...
        cbp = (char *) bp + 28; // + (char *) 28;
	/* Convert the request into a reply. */
	bp->bp_op = BOOTREPLY;
	//if (bp->bp_ciaddr.s_addr == 0) {   // actually ok
        ibp = (int *)bp + 3;
	if (*ibp  == 0) {
		/*
		...
		// bcopy(bp->bp_chaddr, dummyhost.haddr, hlen);
                cbp = (char *) bp + 28;
		bcopy(cbp, dummyhost.haddr, hlen);
		// hashcode = hash_HashFunction(bp->bp_chaddr, hlen);
		hashcode = hash_HashFunction(cbp, hlen);
...
HW addr type is IEEE 802.  convert to %s and check again\n",
		...
		dummyhost.iaddr.s_addr =  *ibp;
		hashcode = hash_HashFunction((u_char *) (ibp), 4);
	 ...
	 */
	 ...
	 //	hp->bootserver.s_addr : 0L;
         ibp = (int *)bp + 5;
         *ibp = (hp->flags.bootserver) ?  hp->bootserver.s_addr : 0L;
	 ...
	 */
        /*
         * Modification for Varian, if file is spec'd and in /tftpboot
	...
	int32 dst_override;
{
	 ...
	 */
        ...
        ibp = (int *)bp; 
	if (dst_override) {
                if (debug>2) report(LOG_INFO,"sendreply() dst_override");
	...
	// } else if (bp->bp_ciaddr.s_addr) {
                ...
                if (debug>2) report(LOG_INFO,"sendreply() copy ciaddr");
                // dst = bp->bp_ciaddr.s_addr
		dst.s_addr = *(ibp+3);
	// } else if (bp->bp_giaddr.s_addr && forward == 0) {
	} else if (*(ibp+6) && forward == 0) {
                if (debug>2)report(LOG_INFO,"sendreply() copy giaddr");
		dst.s_addr = *(ibp+6);
		port = bootps_port;
		if (debug > 1) {
			report(LOG_INFO, "sending reply to gateway %s",
                ...
                if (debug>2)report(LOG_INFO,"sendreply() other");
		// dst = bp->bp_yiaddr;
		dst.s_addr = *(ibp+4);
		//ha = bp->bp_chaddr;
		ha = (cbp+28);
	...
	int32 bootsize;
{
	...
	if (pktlen < sizeof(*bp)) {
		if (debug > 2)
                   report(LOG_ERR, "invalid response length=%d, adjusting", pktlen);
		pktlen = sizeof(*bp);
	}
