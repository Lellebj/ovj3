...
/*	31.3.94  hn	adapted to 2D series reconstruction	*/
/*	31.3.94  mst	messages from \b to '.' changed		*/
/*								*/
/****************************************************************/
#include	"bp.h"
...
{
/*
 * step 0:  set up parameter for 2d BP using the file given
 *          on the command line
 ...
 *	make n_theta 2D reconstructions
 */
char		*par_file;
char		*par_string;
 ...
 */
if (argc == 1) {
    printf ("usage: bp_2d commandfile\n");
    exit (0);
    }
printf ("bp_2d  ---  15.3.1994  ---  updated release\n\n");
...
if (strlen(par_string) == 0) { 
    fprintf (stderr,"BP_2D: profile file name not found\n");
    fprintf (stderr,"usage: bp_2d commandfile\n");
    exit (0);
    }
else { 
...
if (strlen(par_string) == 0) { 
    fprintf (stderr,"BP_2D: warning: measurement size not found\n");
    bp.m_size = 0;
    }
else { 
...
if (strlen(par_string) == 0) { 
    fprintf (stderr,"BP_2D: warning: image size not found, set to measurement size\n");
    bp.i_size = 0;
    }
else { 
...
if (strlen(par_string) == 0) { 
    bp.filter_name = (char *)malloc ((unsigned)strlen("band_pass"));
...
if (strlen(par_string) == 0) { 
    ...
    strcpy (bp.image_file, par_string);
    }
...
/* this is not the raw fid file -- mrk */
 if (access_file (bp.prof_file, &fhp, &bhp, FALSE) == ERROR) {
    fprintf (stderr,"BP_2D: can't access profiles\n");
    return (ERROR);
    }

if (fhp.nblocks * fhp.ntraces != bp.n_theta * bp.n_phi) {
    fprintf (stderr,"BP_2D: mismatch data structure %s and n_theta * n_phi\n",
                     bp.prof_file);
    fprintf (stderr,"n_theta: %d  n_phi: %d\nnblocks: %d ntraces: %d\n",
                     bp.n_theta,bp.n_phi,fhp.nblocks,fhp.ntraces);
    exit (0);
    }
if (bp.m_size == 0) {
    bp.m_size = fhp.np;
...
if (in_memory_prof == TRUE) {
    if ((p_prof_float_start = (float *) malloc ((unsigned) bp.n_theta *
        ...
        fprintf (stderr,"BP_2D: can't allocate profile buffer\n");
        return (ERROR);
        }
    /* this is not the raw fid file - mrk */
    if (read_vnmr_data (bp.prof_file, &fhp, 0,
        ...
        fprintf (stderr,"BP_2D: can't read to profile buffer\n");
        return (ERROR);
        }
    }
else {
...
bp.filter_amp = 1.0 / ((float)(bp.m_size * bp.n_phi));
 if (gen_filter_float (bp.m_size, (float)(bp.m_size)/2.0, &p_filt_float, &bp) == ERROR) 
	{
    fprintf (stderr,"BP_2D: can't first generate filter\n");
    return (ERROR);
    }
...
if ((bound = (struct boundary *) malloc ((unsigned) bp.i_size * 
    ...
    fprintf (stderr,"BP_2D: can't allocate bound buffer\n");
    return (ERROR);
    }
...
if (in_memory_prof == FALSE) {
    if (create_file (bp.prof_filt1, &fhf1,&bhf1) == ERROR) {
        fprintf (stderr,"BP_2D: can't create filtered profile file\n");
        return (ERROR);
        }
    }
...
for (n_slices=last=0; n_slices<bp.n_theta; n_slices++) {
    ...
    if (in_memory_prof == FALSE) {
        if (read_vnmr_data (bp.prof_file, &fhp, n_slices * bp.n_phi,
            ...
            fprintf (stderr,"BP_2D: can't read %d profile slice\n",
                     n_slices);
            return (ERROR);
            }
        }
    ...
    if (in_memory_prof == TRUE) {
        ...
        if (write_vnmr_data (bp.prof_filt1, &fhf1, n_slices * bp.n_phi,
            ...
            fprintf (stderr,"BP_2D: can't write %d filtered profiles\n");
            return (ERROR);
            }
        } 
...
if ((p_image_int = (int *) malloc ((unsigned) bp.i_size * bp.i_size *
             sizeof(int))) == 0) {
    fprintf (stderr,"BP_2D: can't allocate image buffer\n");
    return (ERROR);
    }
...
/* step 3.5: set up file/block header adapted to 2D case */
fhi.nblocks   = 1;
...
if (in_memory_prof == FALSE) {
  if (access_file (bp.prof_filt1, &fhf1,&bhf1, FALSE) == ERROR) {
        fprintf (stderr,"BP_2D: can't access prof_filt1\n");
        return (ERROR);
        }
    }
...
for (n_slices=last=0; n_slices<bp.n_theta; n_slices++) {
    ...
    if (in_memory_prof == TRUE) {
        ...
        if (read_vnmr_data (bp.prof_filt1, &fhf1, n_slices * bp.n_phi,
                                 (n_slices+1) *  bp.n_phi, 1,
                                 (char *)p_prof_float,FALSE) == ERROR) {
            fprintf (stderr,"BP_2D: can't read %d meta slice\n",
                     n_slices);
            }
        }
    ...
    for (i=0; i<bp.i_size * bp.i_size; i++) {
        *(float *)(p_image_int + i) = 
	  (float) abs(*(p_image_int + i));
        }
    /* step 3.6.7: save reconstructed "image" */
    if (n_slices == 0)
    {
    	sprintf (bp.image_file, "P%s.%03d",bp.prof_file, n_slices);
	/* Save Vnmr Phasefile for immediate viewing by Vnmr */
    	if (create_phf_file (bp.image_file, &fhi,&bhi) == ERROR) {
           fprintf (stderr,"BP_2D: can't create phasefile file\n");
           return (ERROR);
           }
    	if (write_phf_data (bp.image_file, &fhi, 0,
           bp.i_size, 1, (char *)p_image_int) == ERROR) {
           fprintf (stderr,"BP_2D: can't write %d image\n", n_slices);
           return (ERROR);
           }
    }
    sprintf (bp.image_file, "%s.%03d",bp.prof_file, n_slices);
    if (create_ib_file (bp.image_file, &fhi,&bhi) == ERROR) {
        fprintf (stderr,"BP_2D: can't create image file\n");
        return (ERROR);
        }
...
if (fgets(par_string, MAX_PAR_STRING, awk_fp) == NULL) {
    fprintf (stderr,"BP_2D: can't access parameter\n");
    *par_string = 0;
    } 
else {
