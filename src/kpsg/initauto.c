...
static char
meth_set_crit(cc)               /* for method shimming ( passes ascii ) */
char            cc;
{
   char            tmp;
...
int shimset;
{
   ...
   while (*pp != '\0')
   {
      switch (*pp++)
      {
         ...
         case 'f':  
	 	     text_error(
			"This system does not support background FID shimming");
...
/* */
   ...
   *out_str++ = DONE_SH;
   ...
   if (com_len == 1)
      ...
      com_len = (com_len +1 ) / 2;  /* two characters per codeint */
   *out_str++ = DONE_SH;
   if (quick_len <= 1)
   {
      quick_len = 0;
   ...
   else
   {
      ...
      *tmpPtr++ = DONE_SH;
      ...
      quick_len = (quick_len +1 ) / 2;  /* two characters per codeint */
   }
