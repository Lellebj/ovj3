...
public class JGears extends GLJPanel {
    ...
    private BufferedImage openglImage = null;
    ...
    public JGears() {
        ...
        try {
            ClassLoader loader = JGears.class.getClassLoader();
            BufferedImage image;
            if (loader != null) {
                InputStream in = loader
                        .getResourceAsStream("vnmr/images/java_logo.gif");
                if (in != null) {
                    image = ImageIO.read(in);
                    javaImage = scaleImage(image, 0.25f, 0.25f);
                }
                in = loader.getResourceAsStream("vnmr/images/opengl_logo.gif");
                if (in != null) {
                    image = ImageIO.read(in);
    ...
    public void paintComponent(Graphics g) {
        ...
        int sp = 10;
        if (javaImage != null)
            g.drawImage(javaImage, getWidth()-javaImage.getWidth()-sp,sp,null);

        if (openglImage != null){
            if(javaImage==null)
                g.drawImage(openglImage, getWidth()-openglImage.getWidth()-sp,sp,null);
            else
                g.drawImage(openglImage, getWidth()-openglImage.getWidth()-javaImage.getWidth()-sp,
                        sp,null);
        }
    }
    ...
    private BufferedImage scaleImage(BufferedImage img, float xScale,
            float yScale) {
        BufferedImage scaled = new BufferedImage(
                (int) (img.getWidth() * xScale),
                (int) (img.getHeight() * yScale), BufferedImage.TYPE_INT_ARGB);
        Graphics2D g = scaled.createGraphics();
        g.setRenderingHint(RenderingHints.KEY_RENDERING,
                RenderingHints.VALUE_RENDER_QUALITY);
        g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
                RenderingHints.VALUE_INTERPOLATION_BICUBIC);
        g.drawRenderedImage(img, AffineTransform.getScaleInstance(xScale,
                yScale));
        return scaled;
    }
