...
/**
 * This Modified version of <code>StreamTokenizer</code> allows quoted
 * strings to contain newlines.  That is, quoted strings can be
 * multiple lines long.  The stock Sun version, terminates quoted
 * strings at the end of the line.  For code changes, search for GRS.
 *
...
public class StreamTokenizerQuotedNewlines {
    ...
    private StreamTokenizerQuotedNewlines() {
	wordChars('a', 'z');
	wordChars('A', 'Z');
     ...
     * @since   JDK1.1
     */
    public StreamTokenizerQuotedNewlines(Reader r) {
	this();
        if (r == null) {
            throw new NullPointerException();
        }
...
//						peekc != '\r') {
// Modified by GRS.  I removed the \n and \r from the original line shown
// above.
	    while (peekc >= 0 && peekc != ttype) {	        
		if (peekc == '\\') {
   		    c = read();
