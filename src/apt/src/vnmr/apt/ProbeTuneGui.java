 ...
 */
/*
 * Copyright (c) 2004-07 Varian, Inc. All Rights Reserved.
 */
...
//                                        "calibration? (press Update Cal Data when done)";
    ...
    protected Stack<Integer> m_previousStatus = new Stack<Integer>();
...
//        cmdSetNumberOfPoints.addActionListener(this);
    ...
    public void displayChannel(String name) {
        String stdName = name.replace('\\', '/');
        String oldName = null;
        Object item = cbChName.getSelectedItem();
        if (item != null) {
            oldName = item.toString();
        }
        if (oldName == null || !oldName.replace('\\', '/').startsWith(stdName)) {
            clearLastStep();
            if (m_sysInitialized) {
                ...
                for (int i = 0; i < nchoices; i++) {
                    ...
                    if (choice.replace('\\', '/').startsWith(stdName)) {
    ...
    private void checkRfCalPermission(String probeName) {
        ...
        if (ProbeTune.useProbeId()) {
            ...
            dir = new File(MtuneControl.getCalDir(probeName));
        }
    ...
    protected void displayStatus(int status) {
        if (status == STATUS_PREVIOUS) {
            m_previousStatus.pop(); // Forget current status
            status = m_previousStatus.pop(); // Revert to previous status
        } else if (status == STATUS_READY) {
            m_previousStatus.clear();
        }
        if (((cmdModeOff.isEnabled() && cmdModeOff.isSelected())
                || (!m_sweepOk || !m_motorOk))
                && status != STATUS_FIRMWARE)
        {
        ...
        case STATUS_CALIBRATING:
            pnlStatus.setBackground(STATUS_CALIBRATING_COLOR);
            lblStatus.setText(STATUS_CALIBRATING_MSG);
            break;
        case STATUS_FIRMWARE:
            pnlStatus.setBackground(STATUS_FIRMWARE_COLOR);
            lblStatus.setText(STATUS_FIRMWARE_MSG);
            break;
        case STATUS_INDEXING0:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING0_MSG);
            break;
        case STATUS_INDEXING1:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING1_MSG);
            break;
        case STATUS_INDEXING2:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING2_MSG);
            break;
        case STATUS_INDEXING3:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING3_MSG);
            break;
        case STATUS_INDEXING4:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING4_MSG);
            break;
        case STATUS_INDEXING5:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING5_MSG);
            break;
        case STATUS_INDEXING6:
            pnlStatus.setBackground(STATUS_INDEXING_COLOR);
            lblStatus.setText(STATUS_INDEXING6_MSG);
            break;
        }
        m_previousStatus.push(status); // Remember new status
    }
...
//    }
    ...
    public void exec(String cmd) {
        //Messages.postDebug("ProbeTuneGui: got command \"" +cmd);
...
//            }
            ...
            if (toker.countTokens() > 0) {
                txtCmd.setText(toker.nextToken("").trim());
            }
    ...
    protected void saveScreenShot(String path) {
        ...
        // image now shows the panel
        new File(path).mkdirs();
        File file = new File(path, "screenshot.png");
        //Messages.postDebug("path=" + file.getAbsolutePath());/*CMP*/
        try {
    ...
    class MotorPanel extends JPanel implements ActionListener {
        ...
        private JLabel mm_positionBarText = new JLabel("-");
