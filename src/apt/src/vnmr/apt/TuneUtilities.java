...
public class TuneUtilities {
    ...
    public static synchronized boolean writeFileSafely(String path,
        ...
        boolean ok = writeFile(tmppath, buffer);
        ...
        if (ok) {
            ...
            int ntrys;
            ok = false;
            for (ntrys = 100; !ok && ntrys > 0 && !tmpfile.renameTo(file); ntrys--) {
                Messages.postError("Failed to rename file on try "
                        + (ntrys - 99) + ": " + tmpfile + " to " + file);
     ...
     * @return True if the write succeeded.
     */
    public static boolean appendLog(String path, String buffer) {
        return appendLogString(path, buffer + NL);
    }

    /**
     * Append the given string to a file with no extra newline.
     * Thread safe.
     * @param path Pathname of the file.
     * @param buffer The string to write.
     * @return True if the write succeeded.
     */
    public static synchronized boolean appendLogString(String path,
                                                       String buffer) {
    ...
    public static int getCountsPerRev(String path) {
        int rtn = 200; // Default counts/rev
            ...
            try {
                while ((line = input.readLine()) != null) {
                    ...
                    if (toker.hasMoreTokens()) {
                        ...
                        if (key.equals("steps/rev")) {
                            rtn = Integer.parseInt(toker.nextToken());
                            break;
                        }
                    }
                }
