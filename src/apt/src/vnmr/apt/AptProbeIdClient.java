...
public class AptProbeIdClient implements ProbeId, ProbeIdIOCommands {
	...
	public int read(byte [] result, String key) {
		try {
			...
			error(iam+"read: socket I/O error");
			e.printStackTrace();
			return 0;
		}
	}
	...
	public void write(String data) {
		try {
			...
			error(iam+"write: socket file not found "+e.getMessage());
			e.printStackTrace();
		} catch (IOException e) {
			error(iam+"write: socket I/O error "+e.getMessage());
			e.printStackTrace();
		} finally {
			closeWriter();
		}
	...
	public void quit() {
		try {
			...
			error(iam+"quit: I/O error "+e.getMessage());
			e.printStackTrace();
		} finally {
			try {
				m_writer.close();
	 ...
	 * @param user - indicates whether it is a user file
	...
	public AptProbeIdClient() throws FileNotFoundException {
        ...
        Messages.postDebug("Initialization", "ProbeTune<probeid.lock>");
	}
	
	AptProbeIdClient(int port) {
		m_port = port;
	}
}
