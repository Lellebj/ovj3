    ...
    public ProbeSimulator(String probe, String sysdir, String chanName) {
        ...
        for (File chanFile : chanFiles) {
            ...
            if (tp != null) {
                ...
                // Each frequency-dependent switch will roughly track the
                // tune-cap position. However, this is not reflected in the
                // frequency sensitivity of the tune cap. So we boost the
                // tune cap sensitivity to give the proper tuning range, at
                // the expense of not giving a realistic sensitivity during
                // tuning. It would be better to make the tune freq depend
                // explicitly on the switch position, but frequency dependent
                // switches are only supported as a legacy feature.
    ...
    public static class RefResonance {
        ...
        public CoilParams getCoilParams(int[] mtrPositions) {
            ...
            for (CoilParamSensitivity cps : mm_coilParamSensitivities) {
                int gmi = cps.gmi;
                if (gmi < 0 || gmi >= mtrPositions.length) {
...
//            System.err.println("Channel " + mm_channelInfo.getChannelNumber()
//                               + ": deltaTune="
//                    + ((coilParams.cTune - mm_coilParams.cTune)
//                            / mm_coilParams.cTune)
//                            + ", deltaCMatch="
//                            + ((coilParams.cMatch - mm_coilParams.cMatch)
//                                    / mm_coilParams.cMatch)
//                    );
            return coilParams;
        }
        ...
        public boolean isResonanceActive(int[] mtrPositions,
            ...
            for (SwitchPosition sp : switchList) {
                ...
                int diff = mtrPositions[gmi] - posn; // Is switch in position?
                if (Math.abs(diff) > 200) { // If posn=0, can't get very close
//                    System.err.println("Resonance NOT active on chan " +
//                            mm_channelInfo.getChannelNumber()
//                            + " switch posn=" + mtrPositions[gmi]
//                            + ", require " + posn);
                    return false;
                }
            }
//            System.err.println("Resonance active on chan " +
//                    mm_channelInfo.getChannelNumber());
            return true;
        }
