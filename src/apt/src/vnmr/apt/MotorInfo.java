...
public class MotorInfo implements Serializable {
    ...
    /**
     * Last odometer value read from the motor.
     */
    private int m_odometer = -1;
    ...
    /** The signed amount of rebound after the last step command. */
    private int m_lastRebound = 0;
    ...
    /** Is operation without indexing allowed? */
    private boolean m_isIndexingOptional = false;
     ...
     * @param countsPerRev TODO
     * @throws InterruptedException If this command is interrupted.
     */
    public MotorInfo(ProbeTune master, int motorNumber, int countsPerRev)
            throws InterruptedException {
        m_master = master;
        m_gmi = motorNumber;
        m_stepsPerRev = countsPerRev;
     ...
     * @param stepsPerRev TODO
     */
    public MotorInfo(int motorNumber, String persistenceDir, int stepsPerRev) {
        m_gmi = motorNumber;
...
//    }
    ...
    public void writePersistence() {
        ...
        if (m_isIndexingOptional) {
            sb.append("NoIndex" + NL);
        }
    ...
    public boolean readPersistence(String path) {
        ...
        double stepFactor = (double)m_stepsPerRev / fileCountsPerRev;
            ...
            while ((line = input.readLine()) != null) {
                ...
                if (toker.hasMoreTokens()) {
                    ...
                    if (key.equals("backlash")) {
                        ...
                        m_maxBacklash *= stepFactor;
                        if (toker.hasMoreTokens()) {
                            m_minBacklash = Double.parseDouble(toker.nextToken());
                            m_minBacklash *= stepFactor;
                        }
                    ...
                    //    m_stepsPerRev = Integer.parseInt(toker.nextToken());
                        ...
                        m_indexDistance *= stepFactor;
                    } else if (key.equals("noindex")){
                        m_isIndexingOptional = true;
...
//    }
    ...
    /**
     * The odometer value that was read from the motor.
     */
    ...
    public void setRebound(int rebound) {
        m_lastRebound = rebound;
    }

    public int getRebound() {
        return m_lastRebound;
    }
    ...
    public boolean isIndexingOptional() {
        return m_isIndexingOptional;
    }

    public int degToSteps(double deg) {
        int steps = (int)Math.round(deg * getStepsPerRev() / 360);
        return steps;
    }
}
