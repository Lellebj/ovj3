...
public class Messages implements AptDefs {
    ...
    /**
     * Post a warning message and record it in the debug log.
     * @param wantBeep If true, and beeping is enabled, VnmrJ will beep.
     * @param msg The message.
     */
    ...
    static private String makeSafeForMagical(String msg) {
        msg = msg.replaceAll("'", "\"");
        if (isSafeForMagical(msg)) {
            return msg;
        }
        ...
        for (int i = len - 1; i >= 0; i--) {
            int chr = sb.charAt(i);
            if (!isSafeForMagical(chr)) {
                sb.replace(i, i + 1, "?");
            }
        }
        String newMsg = sb.toString();
        if (!msg.equals(newMsg)) {
            Messages.postDebug("makeSafeForMagical: from: \"" + msg
                               + "\" to: \"" + newMsg + "\"");
        }
        return newMsg;
    }

    static private boolean isSafeForMagical(String msg) {
        boolean ok = true;
        int len = msg.length();
        for (int i = 0; i < len && ok; i++) {
            ok = isSafeForMagical(msg.charAt(i));
        }
        return ok;
    }

    static private boolean isSafeForMagical(int chr) {
        return (chr >= ' ' && chr <= '~') || chr == '\t';
    }
