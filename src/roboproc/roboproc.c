...
extern int setupAbortRobot(void);                    /* robofuncs.c     */
extern int setSampChngrType(char *type);             /* robofuncs.c     */
...
#define MAXPATHL 256
static char smsPortPath[MAXPATHL];

MSG_Q_ID pRecvMsgQ;
char ProcName[256];
char HostName[80];
char systemdir[MAXPATHL];       /* vnmr system directory */
char roboErrLog[MAXPATHL];
...
int main(int argc, char *argv[])
{
    ...
    if ( ! access("/vnmr/acqbin/Robolog",F_OK) )
    {
       DebugLevel = 1;
    }
    ...
    strcpy(smsPortPath,systemdir);      /* Check if log file is requested */
    strcat(smsPortPath,"/tmp/robolog");
    if ( ! access(smsPortPath,W_OK) )
    {
        logFD = fopen(smsPortPath,"a");
    }
    strcpy(roboErrLog,systemdir);      /* Check if log file is requested */
    strcat(roboErrLog,"/tmp/roboerrlog");

    strcpy(smsPortPath,systemdir);      /* typically /vnmr/smsport */
    strcat(smsPortPath,"/smsport");
    DPRINT1(1,"smsPortPath: '%s'\n",smsPortPath);
    ...
    /* initialize Sample Change Abort Signal */
    setupAbortRobot();

    /* Setup Device Communication */
    smsDevEntry = setupSmsComms(smsPortPath);
    setSampChngrType(smsDevEntry->devName);
...
/*
    strtok("get 110500042"," ");
    getSample("110500042");
    strtok("put 110500042"," ");
    putSample("110500042");
*/
