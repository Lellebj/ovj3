 ...
 */
/* rna_SSnoesy -  2D cross relaxation experiment using S or SS "read" pulse
     ...
     Axial peaks are displaced via FAD for single transient spectra
                  ...
                  DETAILED INSTRUCTIONS FOR USE OF rna_SSnoesy


    1. These Detailed Instructions for rna_SSnoesy may be printed using:
                                      "printon man('rna_SSnoesy') printoff".
...
/* BEGIN PULSE SEQUENCE */
   ...
   status(C);
      ...
      obspower(SSpwr-12.0);
      rgpulse(pwa,v7,2.0e-6,0.0e-6);    /* steering pulses */
      rgpulse(pwd,v8,0.0e-6,rof2);
   status(D);
}
