...
/*  ghn_cacbCTP.c
    ...
    Constant time evolution for 13C shifts
    ...
    ignored if ni=0, and f2180='y' is ignored if ni2=0.
          	  ...
          	  DETAILED INSTRUCTIONS FOR USE OF ghn_cacbCTP
                    

    1. Obtain a printout of the Philosopy behind the BioPack development,
       and General Instructions using the macro: 
                                      "printon man('BioPack') printoff".
       These Detailed Instructions for ghn_cacbCTP may be printed using:
                                      "printon man('ghn_cacbCTP') printoff".
             
    2. Apply the setup macro "ghn_cacbCTP".  This loads the relevant parameter set
   ...
   3.  The delay tauCC is set to 0.6 ms for 1D spectra or for 2D/15N spectra
       ...
       TimeCC is the constant time in 13C evolution.
...
double      d2_init=0.0,  		        /* used for states tppi in t1 */
	    ...
	    tauCC = getval("tauCC"), 		   /* delay for Ca to Cb cosy */
            ...
            timeTC = getval("timeTC"),     /* constant time for Ca/b evol.    */
...
/*   INITIALIZE VARIABLES   */
   ...
   widthHd = 2.681*waltzB1/sfrq;  /* bandwidth of H1 WALTZ16 decoupling, 7.3 kHz at 600 MHz */
...
/* CHECK VALIDITY OF PARAMETER RANGES */
    ...
    if (0.5*ni*1.0/(sw1) > timeTC)
       { printf(" ni is too big. Make ni equal to %d or less. \n",
         ((int)((timeTC)*2.0*sw1))); psg_abort(1); }
...
/*   xxxxxxxxxxxxxxxxxxxxxx       13Cab EVOLUTION       xxxxxxxxxxxxxxxxxx    */
	...
	c13pulse("cab", "co", "square", 90.0, t2, 2.0e-6, 2.0e-6);       /* pwS1 */

        decphase(zero);
        delay(tau1+WFG_START_DELAY);
        c13pulse("co", "cab", "square", 180.0, zero, 2.0e-6, 0.0e-6);
        delay(timeTC-WFG_START_DELAY);
