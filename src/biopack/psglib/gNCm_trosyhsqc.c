...
char        f1180[MAXSTR],   		 /* Flag to start c13 t1  @ halfdwell */
	    f2180[MAXSTR],		 /* Flag to start n15 t1  @ halfdwell */
            shape_methyl[MAXSTR];       /* methyl refocussing shape name     */
...
double      tau1,         				         /*  t1 delay */
  ...
  pw_methyl = getval("pw_methyl"),     /* H1 90 degree pulse length at tpwr_methyl */
  tpwr_methyl = getval("tpwr_methyl"),  /* power for the methyl ("Hm90") pulse */
  tpwrsf_methyl = getval("tpwrsf_methyl"), /* fine power adustment for methylH selective pulse */
    ...
    getstr("f1180",f1180);
    getstr("f2180",f2180);
    getstr("shape_methyl",shape_methyl);
...
icosel=-1;
 if (phase1 == 2)  { tsadd(t7,2,4); tsadd(t10,1,4); icosel = 1;}
            else                               {icosel = -1; }
...
/* half dwell time  compensation for N15
	n15halfdwell = 0.0;
        c13hd_tmp =  1.0/(2.0*sw1) - (4.0/PI)*pwC - 2.0*pw;
if((f1180[A] == 'y') && (f2180[A] == 'n') && (ni > 1.0)) {
  n15halfdwell = -kappa*c13hd_tmp;}
 if((f2180[A] == 'y') && (ni > 1.0)) 
	{ 
...
if (tpwrsf_n<4095.0)
	...
	delay(gstab);
   	decrgpulse(pwC, t3, 0.0, 0.0);  
        ...
        delay(tau1);
        sim3pulse(2.0*pw,0.0,pwN,zero,zero,t4,0.0,0.0);
        delay(tau1);
...
/*  xxxxxxxxx     BACK TRANSFER      xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  */
	...
	sim3pulse(pw, 0.0, pwN, two, zero, t7, 0.0, 0.0);  
	...
	dec2phase(one);
        ...
        txphase(one);
        sim3pulse(pw, 0.0, pwN, one, zero, three, 0.0, 0.0);

	/* zgradpulse(0.15*gzlvl3, gt1/10.0);  */
   /* level emirically adjusted so gzlvl1/gzlvl2 ratio same as gNhsqc */
        delay(gt1/10 + gstab);


   /* shaped pulse for methyl flip */
      txphase(zero);
      obspower(tpwr_methyl);
      obspwrf(tpwrsf_methyl);
      shaped_pulse(shape_methyl,pw_methyl,zero,0.0,0.0);
      obspower(tpwr); obspwrf(4095.0);
   /* shaped pulse for methyl flip end*/
      ...
      sim3pulse(2.0*pw, 2.0*pwC, 0.0, zero, zero, zero, 0.0, 0.0);

            zgradpulse(gzlvl2, gt1/10.0);  
      delay(gstab);

   /* shaped pulse for methyl flip  */
      obspower(tpwr_methyl); obspwrf(tpwrsf_methyl);
      shaped_pulse(shape_methyl,pw_methyl,zero,0.0,0.0);
      obspower(tpwr); obspwrf(4095.0);
   /* shaped pulse for methyl flip end*/
