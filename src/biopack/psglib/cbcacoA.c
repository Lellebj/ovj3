...
status(B);
        obspwrf(pwrCAB);
	shapedpulse("CAB_90",pwCAB,t1,0.0,0.0);
	delay(tau1);
	obspwrf(pwrCO_180);
	...
	shapedpulse("CO_180",pwCO_180,zero,0.0,0.0);
	obspwrf(pwrCAB);
	shapedpulse("CAB_90",pwCA,one,0.0,0.0);
	...
	zgradpulse(gzlvl1,gt1);
        ...
        shapedpulse("CO_180",pwCO_180,zero,0.0,0.0);
        obspwrf(pwrCA);
        shapedpulse("CA_90",pwCA,t2,0.0,0.0);
        ...
        delay(del/2);
/*        obspwrf(pwrCA2);       In the paper but not needed
        shapedpulse("CA_180",pwCA2,zero,0.0,rof2);  	*/
status(C);
	setreceiver(t12);
