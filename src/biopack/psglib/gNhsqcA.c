 ...
 */
/*  gNhsqcA.c
    (Auto-calibrated version, E.Kupce, Varian, 27.08.2002.)
    ...
    Control of Longitudinal Magnetization of aliphatic protons is available.
    ...
    Mz control          K. Pervushin,B.Vogeli and A. Eletsky,JACS,124,12900(2002).
   ...
   The Longitudinal Relaxation Optimized option (Lopt flag). Adiabatic pulses
   on C-13 are used for optimum J(CH) refocussing. The effect is best seen with 
   Lopt='x'('y'). The 'x' or 'y' option puts the (C)H-1 magnetization along -Z instead of +Z.
   When Lopt = 's', a selective EBURP-1 pulse covering CH resonances from -2 to 5.0 ppm 
   is used instead of the conventional H2Osinc flip-back pulse. The water suppression 
   is less efficient.
   (Eriks Kupce, Varian,based on Ref. K. Pervushin,B.Vogeli and A. Eletsky,JACS,124,12900(2002).)
    ...
    set f1180='n' for (0,0) in N15.  f1180='y' is ignored if ni=0.
          	  ...
          	  DETAILED INSTRUCTIONS FOR USE OF gNhsqcA

         
    1. Obtain a printout of the Philosopy behind the BioPack development,
       and General Instructions using the macro: 
                                      "printon man('BioPack') printoff".
       These Detailed Instructions for gNhsqcA may be printed using:
                                      "printon man('gNhsqcA') printoff".
             
    2. Apply the setup macro "gNhsqcA".  This loads the relevant parameter set
   ...
   10. T1rho MEASUREMENTS OF NH GROUPS:
       ...
       VARIABLES section of gNhsqcA.c) causes substantial sample or coil heating 
  ...
  18. The autocal and checkofs flags are generated automatically in Pbox_bio.h
  ...
  19. The Longitudinal Relaxation Optimized option (Lopt flag). Adiabatic pulses
       on C-13 are used for optimum J(CH) refocussing. The effect is best seen with 
       Lopt='x'('y'). The 'x' or 'y' option puts the (C)H-1 magnetization along -Z instead of +Z.
       When Lopt = 's', a selective EBURP-1 pulse covering CH resonances from -2 to 5.0 ppm 
       is used instead of the conventional H2Osinc flip-back pulse. The water suppression 
       is less efficient.
       Ref. K. Pervushin,B.Vogeli and A. Eletsky,JACS,124,12900(2002).

       Eriks Kupce, Oxford, 06/08/2003. 
...
static double   H1ofs=4.7, C13ofs=70.0, N15ofs=120.0, H2ofs=0.0;

static shape H2Osinc, stC200, wuC200;
...
char      f1180[MAXSTR],   		      /* Flag to start t1 @ halfdwell */
	    ...
	    Lopt[MAXSTR],			        /* insert T2 relaxation delay */
...
/* and is called directly from your shapelib.                  		      */
   ...
   rfwu,	                           /* fine power for the wuCall pulse */
	...
	rf0 = 4095.0;    rfst=0.0;  rfwu=0.0;
      ...
      else        /* if autocal = 'y'(yes), 'q'(quiet), r(read), or 's'(semi) */
      {
        if(FIRST_FID)                                            /* call Pbox */
        {
          ...
          if ((Lopt[A] == 'y') || (Lopt[A] == 'x'))
          {
            ppm = getval("dfrq"); ofs=0.0; pws = 0.000650; bw = 160.0*ppm;
            wuC200 = pbox_makeA("wuC200", "cawurst-20", bw, pws, ofs, compC*pwC, pwClvl, 1625);
          }
          if (Lopt[A] == 's')
          {                   /* pwHs parameter is "borrowed" for Pbox */
            ppm = getval("sfrq"); ofs = -2.77*ppm; pwHs = 7.5*ppm;
            H2Osinc = pbox_Rsh("H2Oeb1", "eburp1_", pwHs, ofs, compH*pw, tpwr);
          }
          else
        ...
        if ((Lopt[A] == 'y') || (Lopt[A] == 'x')) rfwu = wuC200.pwrf;
...
/* CHECK VALIDITY OF PARAMETER RANGES */
  ...
  { if(FIRST_FID) text_error( " NHonly='y' option ignored in TROSY exp-ts\n"); 
      NHonly[A]='n'; }
...
/*  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  */
	...
	if((Lopt[A] == 'y') || (Lopt[A] == 'x'))
	{
	  delay(lambda - gt0 - 0.65e-3);
	  decshaped_pulse("wuC200", 0.65e-3, zero, 0.0, 0.0);
	}
	else
	  delay(lambda - gt0);
	...
	txphase(two);
      ...
      if((Lopt[A] == 'y') && (C13refoc[A]=='y'))
      {
	  if ( (NH2only[A]=='y') || (T1[A]=='y') || (T1rho[A]=='y') || (T2[A]=='y') ||
	     (tau1 > 0.5e-3 + WFG2_START_DELAY) ) 
            shaped_pulse(H2Osinc.name, pwHs, two, 5.0e-5, 0.0);
          else
            simshaped_pulse(H2Osinc.name, "wuC200", pwHs, 0.65e-3, two, zero, 5.0e-5, 0.0);
          decpwrf(rfst);
      }
      else if((Lopt[A] == 'x') && (C13refoc[A]=='y'))
      {
	  if ( (NH2only[A]=='y') || (T1[A]=='y') || (T1rho[A]=='y') || (T2[A]=='y') ||
	     (tau1 > 0.5e-3 + WFG2_START_DELAY) ) 
	    simshaped_pulse(H2Osinc.name, "wuC200", pwHs, 0.65e-3, two, zero, 5.0e-5, 0.0);            
...
else if (TROSY[A]=='y')
{
	...
	txphase(three);
      ...
      if (tpwrsf<4095.0) obspwrf(tpwrsf); 
   	shaped_pulse(H2Osinc.name, pwHs, three, 5.0e-5, 0.0);
	obspower(tpwr); 
      if (tpwrsf<4095.0) obspwrf(4095.0);
	txphase(t4);
	delay(5.0e-5);
...
/*  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  */
	...
	{
	  decpwrf(rfwu);
	  if (TROSY[A]=='y')   delay(lambda - 0.65*(pw + pwN) - gt5 - 0.65e-3 - POWER_DELAY);
	  else   delay(lambda - 1.3*pwN - gt5 - 0.65e-3 - POWER_DELAY);
	  decshaped_pulse("wuC200", 0.65e-3, zero, 0.0, 0.0);
	}
	else
        {	
	...
	if((Lopt[A] == 'y') || (Lopt[A] == 'x'))
	{
	  delay(lambda - 1.3*pwN - gt5 - 0.65e-3);
	  decshaped_pulse("wuC200", 0.65e-3, zero, 0.0, 0.0);
	}
	else
