 ...
 */
/*  rna_gCtrosyA.c - autocalibrated version with pulse shaping "on fly".
...
static shape  H2Osinc, stC30, stC50, stC140;
...
/* CHECK VALIDITY OF PARAMETER RANGES */
  ...
  if ((CT[A]=='y') && (S3[A]=='n') && (ni/(4.0*sw1) > CTdelay))
  { text_error( " ni is too big. Make ni equal to %d or less.\n",
      ((int)(CTdelay*sw1*4.0)) ); psg_abort(1); }
...
/*  CHOICE OF PULSE SEQUENCE */
  ...
  if ( ((ribose[A]=='y') && (allC[A]=='y')) || ((ribose[A]=='y') && (aromatic[A]=='y'))
   ...
   { text_error("Choose  ONE  of  ribose='y'  OR  aromatic='y'  OR  allC='y' ! ");
        psg_abort(1); }
...
/*  xxxxxxxxxxxxxxxxxxx    OPTIONS FOR C13 EVOLUTION    xxxxxxxxxxxxxxxxxxxx  */
   ...
   else
   {
    ...
    decphase(zero);
          ...
          if (tau1 < gt1 + grecov)
               {delay(CTdelay/2.0 - 2.0*pwN - gt1 - grecov);
                zgradpulse(gzlvl1, gt1);         /* 2.0*GRADIENT_DELAY */
                delay(grecov - 2.0*GRADIENT_DELAY);
                dec2rgpulse(2.0*pwN, zero, 0.0, 0.0);
                delay(tau1);}

          else {delay(CTdelay/2.0 - 2.0*pwN);
                dec2rgpulse(2.0*pwN, zero, 0.0, 0.0);
                delay(tau1 - gt1 - grecov);
                zgradpulse(gzlvl1, gt1);         /* 2.0*GRADIENT_DELAY */
                delay(grecov - 2.0*GRADIENT_DELAY);}
...
/*  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  */
	...
	delay(lambda - 0.65*(pw + pwC) - gt5);

        simpulse(2.0*pw, 2.0*pwC, zero, zero, 0.0, 0.0);

        zgradpulse(gzlvl5, gt5);
        decphase(t11);
        delay(lambda  - 1.3*pwC - gt5);
  ...
  if(S3[A]=='y')
   {
        ...
        delay(lambda - pwC - gt1/4 - 2.0*POWER_DELAY);
...
status(C);
  ...
  else
   {
	...
	delay(grecov/2 - 0.65*pw + POWER_DELAY);
