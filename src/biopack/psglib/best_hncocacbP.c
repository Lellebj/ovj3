 ...
 */
/*
   BEST-HNCOCACB 1H-15N-13CA/Cb experiment   
   
   Correlates Ca(i) and Cb(i) with N(i+1), NH(i+1).

 The CA->CB transfer delay tauCC can be tuned to 1/(8*35)=3.5ms for
 partial transfer (CACB-type experiment) or to 1/(4*35)=7.1ms for
 complete transfer ((CA)CB-type). CA and CB cross-peaks are of opposite
 sign. Therefore, 1D tests should be carried out with tauCC=gt3 (only
 CA).
...
else
{
  if (phase2 == 2) {tsadd(t3,1,4); icosel =1;}
   ...
   if(t1_counter % 2)
        { tsadd(t1,2,4);tsadd(t2,2,4);  tsadd(t12,2,4); tsadd(t13,2,4); }
...
/**************************************************************************/
        ...
        c13pulse("cab", "co", "square", 180.0, zero, 2.0e-6, 2.0e-6);
         sim3_c13pulse(shname1, "co", "ca", "sinc", "", shpw1, 180.0, 0.0,
