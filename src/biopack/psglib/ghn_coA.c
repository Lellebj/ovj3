...
/*  ghn_coA.c 
    ...
    Modified N15 evolution so that if the number of increments exceeds the CT delay the 
    experiment will be run as semi-constant time evolution. Marco Tonelli @NMRFAM (2004)
   ...
   13. The autocal and checkofs flags are generated automatically in Pbox_bio.h
   ...
   14. PROJECTION-RECONSTRUCTION experiments:  
...
int         icosel,          			  /* used to get n and p type */
            ...
            PRexp = 0,                /* projection-reconstruction flag */
	    ni2 = getval("ni2");
...
/* by Pbox at the 'go' and 'dps' time */
   ...
   compC = getval("compC"),       /* adjustment for C13 amplifier compression */
   	...
   	tpwrd,	  	                           /* rf for WALTZ decoupling */
        waltzB1 = getval("waltzB1"), /* waltz16 rf field strength (in Hz)   */
...
/*   MAKE SHAPES AND INITIALIZE VARIABLES   */
   ...
   /* Example of semi-automatic calibration - use parameters, if they exist : 
        ...
        { 
          if (find("pwC6") > 0) pwC6 = getval("pwC6");
          if (find("rf6") > 0) rf6 = getval("rf6");
        }
   */
      }
...
/* CHECK VALIDITY OF PARAMETER RANGES */
    ...
    if ( (pwC > 100.0e-6) && (ni>1 || ni2>1))
       { printf(" pwC too long! recheck value "); psg_abort(1);} 
...
else if ((ni==1.0) && (pwC3==1.0e-6))        /* 13CO evolution for dof calib. */
   {
 	decpwrf(rf8);
...
{    if (tau2 > gt1 + 2.0*GRADIENT_DELAY + 1.5e-4 + pwHs)
    ...
    else if (tau2 > pwHs + 0.5e-4)
	{
	  ...
	  txphase(t4);
          obspower(tpwr);
          if (tpwrsf<4095.0) obspwrf(4095.0);		       /* POWER_DELAY */
	    delay(0.50e-4 - pwr_dly);
	}
    else
	{
	  txphase(three);
          ...
          obspower(tpwrs);
          if (tpwrsf<4095.0) obspwrf(tpwrsf);		/* POWER_DELAY */
...
else
{
    if (tau2 > kappa)
	{
	  ...
	  obspower(tpwr);				       /* POWER_DELAY */
          delay(kappa -gt1 -2.0*GRADIENT_DELAY -POWER_DELAY);
	}
    else if (tau2 > (kappa - pwC3a - WFG_START_DELAY))
	{
	  ...
	  obspower(tpwr);				       /* POWER_DELAY */
          delay(kappa -pwC3a -WFG_START_DELAY -gt1 -2.0*GRADIENT_DELAY -POWER_DELAY);
	}
    else if (tau2 > gt1 + 2.0*GRADIENT_DELAY + 1.0e-4)
	{
	  ...
	  obspower(tpwr);				       /* POWER_DELAY */
          delay(tau2 -gt1 -2.0*GRADIENT_DELAY -POWER_DELAY);
	}
    else
	{
          delay(timeTN + tau2 - kappa - PRG_STOP_DELAY - pwHd - 2.0e-6);
