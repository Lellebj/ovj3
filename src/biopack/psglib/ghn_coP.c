...
/*  ghn_co.c
    ...
    to gNhsqc. Shaped pulses calculated within pulse sequence code, Jan 99 
    (Version March 99).

    Made waltz decoupling field strength enterable (waltzB1) in Hz. (GG jan03).
    ...
    4. The normal 13C 180 degree pulse on Ca at the middle of t1 induces a
       ...
       will be obtained with pwS=180. As in point 5, a more sensitive 
       comparison of the two spectra with pwS=0,180 can be obtained with
       ...
       Alternatively, change the calibration by changing the phshift parameter
       in the INITIALIZE VARIABLES section of the code.  NOTE THAT dof MUST BE 
       ACCURATELY CALIBRATED (to 1ppm) BEFORE THE phshift CALIBRATION. 
       S/N can also be maximized by arraying compC when (ni=1 pwS=180).
    ...
    5. dof may be calibrated accurately to the center of the CO region, 
       ...
       (1/dfrq*80). Set (ni=1 pwS=1) and obtain a well-phased absorption mode
       ...
       also to be used for the phshift check as in point 4, it is handy to set
       dof using this method first before resetting pwS=180.  Note also that
       dof can be set with more sensitivity with (ni=1 pwS=180) because there
    ...
    7. H2O preservation is achieved according to Kay et al, except that a sinc
       ...
       decoupling power is chosen by changing the strength parameter, waltzB1.
       The new value is specified in the shapelib file after go or dps.
