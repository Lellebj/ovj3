...
/*  gnoesyCNhsqcA.c
    ...
    because of simultaneous pulses and decoupling (done in setup macro).
...
/* CHECK VALIDITY OF PARAMETER RANGES */
    ...
    {
        printf("dm should be 'nnny' 13C is decoupled in t1 by pulse ");
        psg_abort(1);
    }
    ...
    if( pwN < 25.0e-6 )
    {
        printf("Simultaneous pulses on C and N. Use pwNlvl for pwN>40usec");
        psg_abort(1);
    }
    if( pwC < 12.0e-6 )
    {
        printf("Simultaneous pulses on C and N. Use pwClvl for pwC>12usec");
        psg_abort(1);
    } 
    if( pwC > 200.0e-6 )
    {
