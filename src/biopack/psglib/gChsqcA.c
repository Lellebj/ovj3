...
/* gChsqcA.c
    ...
    * Efficient STUD+ decoupling is invoked with STUD='y' without need to 
    ...
    9. Cb COUPLING REFOCUSED WHEN SE='y'; alphaC='y'.
       ...
       respectively.  Then run these two macros (or the macro "biocal") 
  ...
  20. The autocal and checkofs flags are generated automatically in Pbox_bio.h
...
static shape offC2, offC9, offC10, stC12, stC30, stC50, stC80, stC200;
...
double      tau1,         				         /*  t1 delay */
   ...
   ofs, bw, pws, ppm, nst,     /* offset, bandwidth, pulsewidth, ppm, # steps */
...
/* Sech/tanh inversion pulses automatically calculated by macro "biocal"  */
...
/* by the macro "biocal".  SLP pulse shapes, "offC10"/"offC9" are called  */
/* directly from your shapelib.  offC9 uses the same parameters as offC10.    */
   pwC10 = getval("pwC10"),  /* 180 deg pulse at CO(174ppm) null at Ca(56ppm) */
...
/*   INITIALIZE VARIABLES   */
  ...
  if (aliph[A]=='y')          /*  ALIPHATIC spectrum only, centered on 35ppm. */
  {
    if (autocal[0] == 'n') 
    {
    ...
    else        /* if autocal = 'y'(yes), 'q'(quiet), r(read), or 's'(semi) */
    {
      if(FIRST_FID)                                            /* call Pbox */
      {
        ...
        bw = 1000.0*((1.875*sfrq/600.0)+0.31);              /* bandwidth */
        pws = 0.01; ofs = (56.25 - 35.0)*dfrq;                 /* offset */
        stC12 = pbox_makeA("stC12_10", "sech", bw, pws, ofs, compC*pwC, pwClvl, nst);
      }
      rf2 = offC2.pwrf;    pwC2 = offC2.pw;
    ...
    strcpy(stCdec, "stCdec80");      /* 80 ppm STUD+ decoupling */
    stdmf = dmf80;
    studlvl = pwClvl + 20.0*log10(compC*pwC*4.0*rf80);
    studlvl = (int) (studlvl + 0.5);
  }      
  if (alphaC[A]=='y')               /*  ALPHA C spectrum, centered on 56ppm */
  {
    if (autocal[0] == 'n') 
    {
    /* 180 degree one-lobe sinc pulse on CO, null at Ca 118ppm away, dof at   */
    ...
    else        /* if autocal = 'y'(yes), 'q'(quiet), r(read), or 's'(semi) */
    {
      ...
      rf2 = offC2.pwrf;    pwC2 = offC2.pw;
      rf10 = offC9.pwrf;   pwC10 = offC9.pw;
    }
    dofa = dof + 21.0*dfrq;
    strcpy(offCshape, "offC9");
  }
  if ((arom[A]=='y') || (alphaC[A]=='y'))
  {
    if (autocal[0] == 'n') 
    {
    ...
    strcpy(stCdec, "stCdec30");      /* 30 ppm STUD+ decoupling */
    stdmf = dmf30;
    studlvl = pwClvl + 20.0*log10(compC*pwC*4.0*rf30);
    studlvl = (int) (studlvl + 0.5);
    if (arom[A]=='y') 
      dofa = dof + 90.0*dfrq; /*  AROMATIC spectrum only, centered on 125ppm */
  }

  if (allC[A]=='y')  /*  TOTAL CARBON spectrum, centered on 70ppm */
  {
    if (autocal[0] == 'n') 
    {
    ...
    strcpy(stCdec, "stCdec140");    /* 140 ppm STUD+ decoupling */
    stdmf = dmf140;
    studlvl = pwClvl + 20.0*log10(compC*pwC*4.0*rf140);
    studlvl = (int) (studlvl + 0.5);
  }

  if (((alphaC[A]=='y') || (aliph[A]=='y')) && 
      ((Crefoc_r[A]=='y') || (Crefoc_l[A]=='y'))) 
  {
    if (autocal[0] == 'n') 
    {
    ...
    else        /* if autocal = 'y'(yes), 'q'(quiet), r(read), or 's'(semi) */
    {
      if(FIRST_FID)                                             /* call Pbox */
      {
        pws = 0.005; nst = 1000.0;    
        bw = 1000.0*((7.5*sfrq/600.0)+0.47);                    /* bandwidth */
        if (Crefoc_r[A]=='y')
        {
          ofs = (-56.0 + 50.0)*dfrq - 430.0 - rfst/2.0;              /* offset */
          stC50 = pbox_makeA("stC50_5r", "sech", bw, pws, ofs, compC*pwC, pwClvl, nst);
        }
        else if (Crefoc_l[A]=='y')
        {
          ofs = (-56.0 + 63.0)*dfrq + 430.0 + rfst/2.0;              /* offset */
          stC50 = pbox_makeA("stC50_5l", "sech", bw, pws, ofs, compC*pwC, pwClvl, nst);
        }
      }
      rfst1 = stC50.pwrf;
    } 
