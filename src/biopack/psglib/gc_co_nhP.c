...
/*  gc_co_nhP.c               
    ...
    ignored if ni=0, and f2180='y' is ignored if ni2=0.
          	  ...
          	  DETAILED INSTRUCTIONS FOR USE OF gc_co_nhP

    1. Obtain a printout of the Philosopy behind the BioPack development,
       and General Instructions using the macro: 
                                      "printon man('BioPack') printoff".
       These Detailed Instructions for gc_co_nhP may be printed using:
                                      "printon man('gc_co_nhP') printoff".
             
    2. Apply the setup macro "gc_co_nhP".  This loads the relevant parameter set
    ...
    3. The parameter ncyc corresponds to the number of cycles of DIPSI-3 mixing.
       ...
       spectrometer is sufficient to cover 14.4 kHz (95 ppm at 600 MHz) and is
       more than adequate for the CC J's.  The bandwidth is set by the spinlock
       value (in Hz). This value is used in the c13decouple(...) statement. The
       new value is specified in the new shapelib file after go or dps. 
       Because the DIPSI3 is being used for a mixing instead of decoupling, Pbox
       needs a higher input bandwidth to achieve the same actual RF field strength.
       This factor is incorporated into the c13decouple statement below.
    ...
    6. The H1 frequency is NOT shifted to the amide region during the sequence.
       H1 decoupling rf field may be chosen by changing the waltzB1 strength.
   ...
   7.  The parameter phi7cal (listed in dg and dg2) is provided to adjust 
       ...
       be calibrated once only for each spectrometer and gc_co_nhP .  The 
       value is the same for ghc_co_nhP and gcbca_co_nhP.
   ...
   10. TROSY:
       ...
       gc_co_nhP so the N15 magnetization has not evolved with respect to the 
...
double      d2_init=0.0,  		        /* used for states tppi in t1 */
   ...
   spinlock = getval("spinlock"),      /*DIPSI-3 decoupling field (Hz) on Cab */
...
/*  Hyperbolic sheila seems superior to original zeta approach  */ 
    ...
    epsilon = epsilon - pwS3 - WFG_START_DELAY - 1.0e-6 - POWER_DELAY 
	      - PWRF_DELAY - gt4 - 2.0*GRADIENT_DELAY - 5.0e-5;
