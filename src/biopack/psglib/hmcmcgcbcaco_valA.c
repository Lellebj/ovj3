...
static shape    rb180, rb180_co, ca90, ca180;
...
/* DECLARE VARIABLES */
 ...
 double      tau1,         /*  t1 delay */
             ...
             pwcrb180,     /* PW180 for C 180 reburp @ rfrb & pwClvl */
...
/* ensure that magnetization originates on 1H and not 13C */
   ...
   decshaped_pulse(rb180_co.name,pwcrb180,zero,4.0e-6,0.0);  /* BS */
   decpwrf(rfrb);
   delay(taub - (2.0/PI)*pwC - POWER_DELAY - 4.0e-6 - WFG_START_DELAY
         - pwcrb180 - WFG_STOP_DELAY - 2.0e-6
         - WFG_START_DELAY);
   ...
   decshaped_pulse(rb180_co.name,pwcrb180,zero,4.0e-6,0.0);
   decphase(t5);
   decpwrf(rfca90); 
   ...
   decshaped_pulse(rb180_co.name,pwcrb180,zero,0.0,0.0);
   decpwrf(rfrb); 
   initval(1.0,v3);
