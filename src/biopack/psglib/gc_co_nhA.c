...
/*  gc_co_nhA.c               
    ...
    1H dipsi2 decoupling strength controllable by waltzB1 parameter (NM jan03)

    Semi-Constant Time option -  Eriks Kupce, Oxford, 26.01.2006, based on  
                                 example provided by Marco Tonelli at NMRFAM
    Sequence modified for Z-mixing and use of waveform spinlocks. E.Kupce,11/2007
    ...
    ignored if ni=0, and f2180='y' is ignored if ni2=0.
          	  ...
          	  DETAILED INSTRUCTIONS FOR USE OF gc_co_nhA

    1. Obtain a printout of the Philosopy behind the BioPack development,
       and General Instructions using the macro: 
                                      "printon man('BioPack') printoff".
       These Detailed Instructions for gc_co_nhA may be printed using:
                                      "printon man('gc_co_nhA') printoff".
             
    2. Apply the setup macro "gc_co_nhA".  This loads the relevant parameter set
    ...
    6. The H1 frequency is NOT shifted to the amide region during the sequence.
       The H1 DIPSI2 decoupling can be controlled by the waltzB1 parameter.
   ...
   7.  The parameter phi7cal (listed in dg and dg2) is provided to adjust 
       ...
       be calibrated once only for each spectrometer and the gc_co_nhA and 
   ...
   10. TROSY:
       ...
       gc_co_nhA so the N15 magnetization has not evolved with respect to the 
   ...
   11. The autocal and checkofs flags are generated automatically in Pbox_bio.h
       ...
       the original code). When autocal<>'n', the spinlock waveform is selectable via
       the "mixpat" parameter. Pbox uses the "spinlock" parameter to calculate the
       attenuator setting. From this it creates the proper waveform to give a
       spinlock field strength of "spinlock" Hz, and sets the proper decspinlock
       parameters. If autocal is 'n', the user can enter all the relevant parameters.    
...
static shape CCmix, offC1, offC2, offC4, offC5, offC7;
...
/* directly from your shapelib.                    			      */
   ...
   phi7cal = getval("phi7cal"),  /* phase in degrees of the last C13 90 pulse */
   	...
   	tpwr1,	  	                 /* 7.3 kHz rf for magnet for DIPSI-2 */
   	DIPSI2time,     	        /* total length of DIPSI-2 decoupling */
        waltzB1 = getval("waltzB1"), /* H1 dipsi2 decoupling strength (in Hz) */
        ncyc_dec,
