...
pulsesequence()
{
   ...
   double d2_init=0.0,
          ...
          lambda1 = getval("lambda1"),
          lambda2 = getval("lambda2"),
          lambda3 = getval("lambda3"),
          lambda4 = getval("lambda4"),
          scale, scale1, 
...
if (ni>1.0)
{
   scale1 = (timeTN-pwS2*0.5-gt2-shpw1)*2.0*sw1/ni;
    putCmd("scale1 = %f')\n",scale1);
}
else  scale1 = 0.0;
...
if (ix == 1)
{ printf("The scaling of the NH coupling is %f.\n",
      ((scale)));
}
if (SE_flg[0] == 'y')
{
...
else
{
   ...
   if ( ni >2 && f1180[0]=='n')
       { printf("  Make f1180 equal to 'y'\n");
          psg_abort(1);}

   if ( 0.5/(sw1) < ((pwS1*2.0/3.0+pwS3*0.5+pwN*0.5)*4.0))
       { printf(" SW1 is too big. Make SW1 equal to %d or less.\n",
         ((int)( 0.5/(pwS1*2.0/3.0+pwS3*0.5+pwN*0.5)/4.0)));    psg_abort(1);}
...
else
{
  if (phase2 == 2) tsadd(t5,1,4);
   ...
   if(t2_counter % 2)
        {tsadd(t5,2,4);tsadd(t12,2,4); tsadd(t13,2,4); }
...
/**************************************************************************/
	...
	c13pulse("co", "ca", "sinc", 90.0, t1, 0.0, 0.0);       
       ...
       if (ni>=1.0)
       {
        delay(tau1*0.25-pwS1*2.0/3.0-pwS3*0.5-pwN*0.5);
	c13pulse("ca", "co", "square", 180.0, zero, 0.0, 0.0);       
        delay(tau1*0.25-pwS3*0.5-pwN*0.5);
        sim3shaped_pulse(shname1,"","", shpw1, 0.0, 2.0*pwN, zero, zero, zero, 0.0, 0.0);
        delay(tau1*0.25-pwS3*0.5-pwN*0.5);
	c13pulse("ca", "co", "square", 180.0, two, 0.0, 0.0);       
        delay(tau1*0.25-pwS1*2.0/3.0-pwS3*0.5-pwN*0.5);
       }
       else
       {
