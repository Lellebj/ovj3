...
/*  ghcch_tocsy.c               
    ...
    3. The parameter ncyc corresponds to the number of cycles of DIPSI-3 mixing.
       ...
       (2 or 3)*6.07*600/sfrq ms for a DIPSI rf field of 9 kHz for a 600Mhz 
...
status(A);
	...
	rgpulse(2.0*pw, zero, 0.0, 0.0);
                ...
                if (mag_flg[A] == 'y')
                {
                   magradpulse(icosel1*gzcal*0.2*gzlvl1,0.5*gt1);
                }
                else
                {
                   zgradpulse(icosel1*0.2*gzlvl1, 0.5*gt1);
                }
        decphase(t5);
	delay(0.5*del - 0.5*gt1);
