...
/* CHECK VALIDITY OF PARAMETER RANGES */
    ...
    if (0.5 * ni2 * 1 / (sw2) > timeNCA - timeC - lambda - gt1 - 1.50e-4 - pwHs)
    {
      printf(" ni2 is too big. Make ni2 equal to %d or less.\n",
        ((int) ((timeNCA - timeC - lambda - gt1 - 1.50e-4 - pwHs) * 2.0 * sw2)));
      psg_abort(1);
    }
    ...
    if (dpwr2 > 46)
    {
      text_error("dpwr2 too large! recheck value  ");
      psg_abort(1);
    }

    if (pw > 20.0e-6)
    {
      text_error(" pw too long ! recheck value ");
      psg_abort(1);
    }

    if (pwN > 100.0e-6)
    {
      text_error(" pwN too long! recheck value ");
      psg_abort(1);
    }
...
status(A);
	...
	txphase(zero);
   	delay(1.0e-5);
        ...
        if (TROSY[A]=='n')	dec2rgpulse(pwN, zero, 0.0, 0.0);  /*destroy N15 and C13 magnetization*/
...
if ((ni>1.0) && (tau1>0.0))          /* total 13C evolution equals d2 exactly */
   {           
     ...
     if (tau1 - 2.0*(sqrt(15.0)/(4.0*128.0*dfrq))/PI - WFG3_START_DELAY - 0.5*pwZ - 2.0e-6
                                - 2.0*PWRF_DELAY - 2.0*POWER_DELAY > 0.0)
           {
        delay(tau1 - 2.0*(sqrt(15.0)/(4.0*128.0*dfrq))/PI - WFG3_START_DELAY - 0.5*pwZ - 2.0e-6 
        ...
        decstepsize(1.0);
        dcplrphase(v3);                                        
        delay(tau1 - 2.0*(sqrt(15.0)/(4.0*128.0*dfrq))/PI  - SAPS_DELAY - 0.5*pwZ - WFG_START_DELAY 
        ...
        decstepsize(1.0);
        dcplrphase(v3);                                        
        delay(2.0*tau1 - 4.0*(sqrt(15.0)/(4.0*128.0*dfrq))/PI - SAPS_DELAY - WFG_START_DELAY - 2.0e-6 
                                                - PWRF_DELAY - POWER_DELAY);
           } 

     /*  delay(tau1);
       sim3_c13pulse("", "co", "ca", "sinc", "", 0.0, 180.0, 2.0*pwN,zero, zero, zero, 2.0e-6, 2.0e-6);
       delay(tau1);
       c13pulse("cab", "co", "square", 180.0, t6, 0.0, 0.0);
       sim3_c13pulse("", "co", "ca", "sinc", "", 0.0, 180.0, 2.0*pwN,zero, zero, zero, 2.0e-6, 2.0e-6); */
   }
