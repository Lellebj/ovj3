...
char        f1180[MAXSTR],   		      /* Flag to start t1 @ halfdwell */
            ...
            refresh[MAXSTR],        /* refresh waveforms - q (quietly), y & n */
...
double      tau1,         				         /*  t1 delay */
	    ...
	    reps,                             /* used to control Pbox output  */
...
/* the sech/tanh pulse is automatically calculated by the macro "proteincal", */  /* and is called directly from your shapelib.                  		      */
   ...
   compC = getval("compC"),         /* adjustment for C13 amplifier compression */
    ...
    getstr("refresh",refresh);
    if(refresh[0] == 'q') reps = 0.0;
    else reps = 2.0;
...
/* 180 degree adiabatic C13 pulse from 0 to 200 ppm */
    ...
    if (C13refoc[A]=='y')
    {
      if (FIRST_FID)      /* calculate the shape only once */
      {
        ...
        {
          opx("Pbox");                                                /* open Pbox */
          pboxpar("maxincr", 10.0);   /* set maximum phase increment to 10 degrees */ 
          pboxpar("reps", reps);                   /* used to control Pbox output  */
          pboxUpar("attn", pwClvl, "E");           /* set course attenuation level */            
          pboxpar("steps", 1000.0);
          rfst = 200.0*dfrq;                        /* bandwidth */
          stC200 = pboxAshape("stC200", "sech", rfst, 0.001, 0.0, compC*pwC, pwClvl);        
        }
        else
          stC200 = getRsh("stC200");        
      }
      rfst = stC200.pwrf;
