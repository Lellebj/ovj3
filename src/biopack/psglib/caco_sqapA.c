...
/*  caco_sqapA.c
    ...
    2D 13C observe CACO - SQ Anti Phase constant time experiment.
...
status(B);
	...
	obspwrf(pwrCaliph2);
	shapedpulse("CA_180",pwCaliph2,t4,0.0,0.0);
	delay(TC/2.0 - tau1);
	obspwrf(pwrCO_180);
	shapedpulse("CO_180",pwCO_180,zero,0.0,0.0);
	obspwrf(pwrCaliph1);
	shapedpulse("CA_90",pwCaliph1,t1,0.0,0.0);
	zgradpulse(gzlvl2*0.8,gt2);
	delay(gstab);
	obspwrf(pwrCO_90);
	shapedpulse("CO_90",pwCO_90,t3,0.0,rof2);
