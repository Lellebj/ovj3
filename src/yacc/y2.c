 ...
 */
#ifndef lint
static char sccsid[] = "y2.c	(Berkeley)	2/11/83";
#endif not lint
...
extern char *TEMPLATE;	/* parser template file */
...
{  int   i,j,lev,t, ty;
   ...
   for (Ia=1; Ia<argc; ++Ia)
      if (*(Pa=argv[Ia]) == '-')
	    ...
	    switch (*Pa)
	    { case 'd':
	      case 'D': fdefine = fopen(FILED,"w");
			break;
	      case 'f':
	      case 'F': if (++Ia < argc)
			   TEMPLATE = argv[Ia];
			else
			{  error("yacc: missing arg for -f");
			   exit(1);
			}
			break;
	      case 'o':
	      case 'O': fprintf(stderr,"-o is now on by default\n");
			break;
	      case 'v':
	      case 'V': if ((foutput=fopen(FILEU,"w")) == NULL)
			   error( "cannot open y.output" );
			break;
	      default:  fprintf(stderr,"yacc: unknown option '%c'\n",*Pa);
			break;
	    }
      else
	 if (0 < fileCount)
	 {  fprintf(stderr,"too many input files\n");
	    ...
	    if ((finput=fopen(infile=argv[Ia],"r")) == NULL)
	    {  fprintf(stderr,"yacc: can't open %s for input\n",argv[Ia]);
	       exit(1);
	    }
	    else
	       fileCount += 1;
   if ((ftable=fopen(OFILE,"w")) == NULL)
      fprintf(stderr,"yacc: cannot open table file (=%s)\n",OFILE);
   if ((ftemp=fopen(TEMPNAME,"w")) == NULL)
      fprintf(stderr,"yacc: cannot open temp file (=%s)\n",TEMPNAME);
   ...
   {  switch (t)
      { case ';':     t = gettok();
		      break;
	case START:   if( (t=gettok()) != IDENTIFIER )
		      {  error( "bad %%start construction" );
		      }
		      start = chfind(1,tokname);
		      t     = gettok();
		      continue;
	case TYPEDEF: if ((t=gettok()) != TYPENAME)
			 error( "bad syntax in %%type" );
		      ty = numbval;
		      for (;;)
		      {  t = gettok();
			 switch (t)
			 { case IDENTIFIER: if ((t=chfind(1,tokname)) < NTBASE)
					    {  j = TYPE( toklev[t] );
					       if( j!= 0 && j != ty )
