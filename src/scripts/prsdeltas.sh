: '@(#)prsdeltas.sh 22.1 03/24/08 1991-1996 '
# 
#
...
echo " "
echo " Obtains the Present deltas for specified categories."
echo " "
echo "Date: `date`"
echo " "
...
# ------ extract all files from SCCS category ------------------
    ...
    if (test -n "$files")
    then
       awk '$0 ~ /\@\(\#\)/ { for (i = 1; i <= NF; i++) 
                               { if ($i ~ /\@\(\#\)/) 
	                         { printf("%s  ",$(i+1)); printf("%-28s ",$i); 
				   printf(" %8s ",$(i+2));
                                    for (j = i+3; j <= NF; j++) { printf("%s ",$j);}
                                      printf("\n") 
                                 }
                               }
                             }
