: /bin/sh
# '@(#)vnmrx_tarout.sh 22.1 03/24/08 1991-1996 '
# 
#
...
#--- tar the IMAGE common files maclib, menulib, parlib, imaging ...
   echo " "
...
#-----   Now time for Object Files
   ...
   tar -cf $1/sun4/uimage.tar $BinImage2Tar
...
mktoc () {    # $1 is the passed directory
   ...
   echo -n "sun4.toc  "
   cp -p $sourcedir/sysscripts/sun4.toc sun4.toc
...
name=`basename $0`
if ( test $name = "vnmrx_tarout" )
then
 echo "Making VnmrX Tar Tape for combined SunView X-Window support"
 taropt=cvfb
 blksiz=2000
 tardev=/dev/nrst8
 tardev1=/dev/nrst8
...
# check for file presents.. 
 ...
 echo "Making VnmrX Tar File \`$tardev' for combined SunView X-window support"
else
...
echo " "  | tee -a $logpath
 ...
 tapetype="t"
   ...
   echo -n "Specify Directory to place Tar Files [$tmptape/sunX]: "
   read tmpdir
   if [ x$tmpdir = "x" ]
   then
     tmpdir=$tmptape/sunX
...
echo "VnmrX tape complete" | tee -a $logpath
exit 0
