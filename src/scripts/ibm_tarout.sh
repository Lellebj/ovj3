: /bin/sh
# '@(#)ibm_tarout.sh 22.1 03/24/08 1991-1996 '
# 
#
...
# 
#
#  Version for support of IBM
#

# ISO quality record log
lognam=VnmrI_tarlog`uname -m`
logdirectory=$logdir
if [ x$logdirectory = "x" ]
then
...
#-----   Now time for Object Files
   echo    " " | tee -a $logpath
   echo    " " | tee -a $logpath
   echo "Generating Tar IBM files" | tee -a $logpath
   if test ! -d $1/ibm
   then
     mkdir $1/ibm
   else
     rm -rf $1/ibm	# remove all previous files
     mkdir $1/ibm
   fi
   cd $ibmobjdir
   echo -n "acqbin  "
   echo "acqbin.tar" >> $logpath
   tar -cf $1/ibm/acqbin.tar $BinAcq2Tar

   echo -n "bin  "
   echo "bin.tar" >> $logpath
   tar -cf $1/ibm/bin.tar $BinFiles2Tar

#  echo -n "bins  "
#  echo "bins.tar" >> $logpath
#  tar -cf $1/ibm/bins.tar $BinSV2Tar

#  echo -n "binx  "
#  echo "binx.tar" >> $logpath
#  tar -cf $1/ibm/binx.tar $BinX2Tar
   ...
   chmod 755   ./seqlib
   chmod 755   ./seqlib/*
   tar -cf $1/ibm/seqlib.tar seqlib
   rm -rf seqlib
   cd $ibmobjdir

   echo "limnet  "
   echo "limnet.tar" >> $logpath
   tar -cf $1/ibm/limnet.tar limnet

#  echo "Glide "
#  echo "glide.tar" >> $logpath
#  tar -cf $1/ibm/glide.tar $BinGlide2Tar

   echo "PFG "
   echo "pfg.tar" >> $logpath
   tar -cf $1/ibm/pfg.tar $BinPFG2Tar
   echo "Image "
   echo "uimage.tar" >> $logpath
   tar -cf $1/ibm/uimage.tar $BinImage2Tar

#  echo "kermit "
#  echo "kermit.tar" >> $logpath
#  cd $commondir
#  tar -cf $1/ibm/kermit.tar $Bin4Kermit2Tar
   cd $ibmobjdir
   (cd $1/ibm; rm -f ibm.sizes; du -s *.tar > ibm.sizes; )
#  (cd $1/ibm; compress binx.tar; )
   ...
   echo    " " | tee -a $logpath
   cd $commondir
   echo "ibm frame3_ibm.tar" >> $logpath
   tar -cf $1/ibm/frame3_ibm.tar $Bin4XFrame32Tar
   (cd $1/ibm; du -s frame3_ibm.tar >> ibm.sizes; )
   (cd $1/ibm; compress frame3_ibm.tar ; )
   echo "ibm frame_ibm.tar" >> $logpath
   tar -cf $1/ibm/frame_ibm.tar $Bin4XFrame2Tar
   (cd $1/ibm; du -s frame_ibm.tar >> ibm.sizes; )
   (cd $1/ibm; compress frame_ibm.tar ; )
}
...
mktoc () {    # $1 is the passed directory
   ...
   echo -n "ibm.toc  "
   cp -p $sourcedir/sysscripts/ibm.toc ibm.toc
   Updatetoc ibm.toc ../ibm/ibm.sizes ibm.toc.awk
   rm -f ibm.toc
   mv ibm.toc.awk ibm.toc
   cp -p $ibmobjdir/proglib/bin/getchoices getchoices.ibm
   echo "vnmros=AIX" > VnmrI_5.1a
...
name=`basename $0`
if ( test $name = "ibm_tarout" )
then
 echo "Making VnmrI Tar Tape for IBM support"
 taropt=cvfb
 blksiz=20
 tardev=/dev/nrst8
 tardev1=/dev/nrst8
...
# check for file presents.. 
 ...
 echo "Making VnmrI Tar File \`$tardev' for IBM support"
else
...
echo " "  | tee -a $logpath
 ...
 echo "ibm files from: $ibmobjdir" | tee -a $logpath
 echo    " " | tee -a $logpath
 echo    " " | tee -a $logpath
 echo
 ...
 then
   ...
   echo "ibm files from env ibmobjdir: $ibmobjdir"
 ...
 tapetype="t"
   ...
   echo -n "Specify Directory to place Tar Files [$tmptape/ibm]: "
   read tmpdir
   if [ x$tmpdir = "x" ]
   then
     tmpdir=$tmptape/ibm
...
#-------------------------------------------------------------------
  if test $nfiles -gt 0
  then
   ls -ts $tmpdir/com/*.*ar* $tmpdir/ibm/*.*ar*
...
#  Third file contains IBM stuff

echo    " " | tee -a $logpath
echo    " " | tee -a $logpath
echo "Writing IBM Files" | tee -a $logpath
...
then
  ...
  rsh $REMOTE_HOST "(cd $tmpdir/ibm; tar $taropt $tardev4 $blksiz $TapeSun4FileLst)"
fi
...
echo "VnmrI tape complete" | tee -a $logpath
exit 0
