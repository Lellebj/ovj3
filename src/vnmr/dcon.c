...
{ int i,ctrace,ithv;
  ...
  for (i=1; i<=npnt1; i++)
	...
	{ case 1: fillit(out+dfpnt,ithv,dnpnt); break;
      ...
      if (dcon_colorflag) stack_changecolor(fpnt1+i);
      grf_batch(1);
  ...
  if (argc>argnum)
    { for (i=argnum; i<argc; i++)
        { if (strcmp(argv[i],"linear")==0) linearflag = 1;
            ...
            { Werrprintf(
                "usage - dcon(x,x,..) where x=linear,phcolor,avcolor,gray");
	      graph_batch(0);
              skyrel(); return 1;
            }
        }
    ...
    case SELECT_AV_COLOR: dcon_first_color = FIRST_AV_COLOR;
			  ...
			  else
			  {
			        ...
			        norm_phaseangle++;
                          ...
                          break;
    case SELECT_PH_COLOR: dcon_first_color = FIRST_PH_COLOR;
                          dcon_num_colors  = NUM_PH_COLORS;
                          if (th>(dcon_num_colors/2) -1)
                            th = (dcon_num_colors/2) -1;
                          break;
    case SELECT_GRAY_COLOR: dcon_first_color = FIRST_GRAY_COLOR;
...
/*********************/
static int filled_display(phflag,pos_only)
/*********************/
int phflag, pos_only;
{ 
