...
typedef struct {
      unsigned long 	adcBaseAddr;
		...
		int     dspPromType;
    		int	index;	/* 0 to nbrOfAdcSlots-1 */
} ADC_OBJ;
...
/* extern ADC_ID  adcCreate(int index, int level, char* idstr); */
extern ADC_ID  adcCreate(int adcChennal);
...
extern void adcShowEm(int level);
extern void adcShow(ADC_ID pAdcId,int level);
extern int adcPutAdcObjByIndex(ADC_ID pAdcObj, int index);
extern ADC_ID adcGetAdcObjByIndex( int index );
extern ADC_ID adcGetActive(ulong_t *activeRcvrs, int *pindex);
extern int adcNumberOfAdcs();
...
extern void adcShowEm();
extern void adcShow();
extern int    adcPutAdcObjByIndex();
extern ADC_ID adcGetAdcObjByIndex();
extern ADC_ID adcGetActive();
extern int adcNumberOfAdcs();
