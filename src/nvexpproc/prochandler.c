...
#define MAXPATHL 128
extern void TheGrimReaper(void* arg);
...
pid_t startTask(char *taskname, char *filepath)
{
    ...
    isSendorRecvproc = (strcmp(taskname,taskList[RECVPROC].procName) == 0) ||
            (strcmp(taskname,taskList[SENDPROC].procName) == 0);

    isStat=(strcmp(taskname,taskList[STATPROC].procName) == 0);
    ...
    else
    {
        ...
        if (isSendorRecvproc == 0 )
            sigprocmask( SIG_SETMASK, &qmask, &omask );
        DPRINT2(1,"STARTING : %s isStat :%d\n", taskname,isStat);
        if(isStat)
            ret = execl(testpath, taskname, "-l", NULL);
        else
...
int startAutoproc(char *autodir, char *autoDoneQ)  /* if not started then start it */
{
   ...
   {
       DPRINT(1,"startATask: Make sure it is still alive\n");
       TheGrimReaper(NULL);
   }
   if (taskList[AUTOPROC].taskPid != -1)
   {
...
static void startTasks()
{
   ...
   for( i = RECVPROC; i < PROC_RECORD_SIZE; i++)   /* just for early testing for now */
   {
      if (taskList[i].autostart == 1)
      {
         ...
         {
	    errLogSysQuit(ErrLogOp,debugInfo,"Can find '%s', Correct then restart.",
	     taskList[i]. procName);
         }
      }
   }
   for( i = RECVPROC; i < PROC_RECORD_SIZE; i++) 
   {
      if (taskList[i].autostart == 1)
      {
       ...
       if (alive != 0)
       {
         sleep(1);   /* stagger the starting of processes (for NDDS) */
...
void killEmAll()
{
   killEmDead(RECVPROC,LASTPROC);
}

char *registerDeath(pid_t pid)
{
   int i;
   for( i = EXPPROC; i <= LASTPROC; i++)
   {
...
{
   DPRINT2(1,"chkTaskActive %d: PID= %d\n",task,taskList[task].taskPid);
   if (taskList[task].taskPid != -1)
   {
       TheGrimReaper(NULL);
   }
   if (taskList[task].taskPid > 0)
   {
      if (kill(taskList[task].taskPid,0))  /* are you out there */
         taskList[task].taskPid = (pid_t) -1;
   }
