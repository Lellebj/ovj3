...
extern int expQaddToHead(int priority, char* expidstr, /*   expQfuncs.c */
                         char* expinfostr);
extern void downloadShims();                           /* conhandler.c */
...
int ipctst(char *argv)
{
    ...
    if (consoleConn())
    {
	sprintf( Msge, "%s%c%s%c%s","ipccontst", DELIMITER_1, returnInterface, DELIMITER_2, token);
        strsize = strlen(Msge) + 1;
        send2Monitor(ECHO, PARSE, 0, 0, Msge, strsize);
...
/*  char  Msge[512]; */
  ...
  int totalq;
  int JpsgFlag; 
  unsigned int nAcodes, nFIDs,startFID;
  ...
  if (consoleConn())
  {
     ...
     if (SystemReady4NextGo != 1)
     {
        DPRINT1(1,"chkExpQ: SystemReady4NextGo: %d, skip exp for now.\n",SystemReady4NextGo);
        return(-1);
     }
		...
		(pSndMsgQ->MsgQDbmEntry.pidActive > 0) )
	     ...
	     startFID = ActiveExpInfo.ExpInfo->Celem;
   	     JpsgFlag = ((nAcodes == 1) && (nFIDs > 1)) ? 1 : 0;
             ...
             /* ActiveExpInfo.ExpInfo->GoFlag = GO 0, SU 1, SHIM 2, LOCK 3, SPIN 4, CHANGE 5, SAMPLE 6 */

   	     DPRINT4(1,"chkExpQ: # Acodes: %u, # FIDs: %u,  Jpsg flag: %d, startAcode: %u\n",
   	     ...
   	     DPRINT1(1,"chkExpQ: GoFlag = %d (GO 0, SU 1, SHIM 2, LOCK 3, SPIN 4, CHANGE 5, SAMPLE 6) \n",
   	     ...
   	     DPRINT1(1,"New startAcode value: %d\n",startFID);
              ...
              Acodes2WaitFor = ActiveExpInfo.ExpInfo->NumAcodes;
              if (ActiveExpInfo.ExpInfo->GoFlag == 0 /* GO */)
			...
			startFID, ActiveExpInfo.ExpInfo->AcqBaseBufName,  strsize);
              ...
              send2Monitor(APARSER, Acodes2WaitFor, ActiveExpInfo.ExpInfo->NumTables, 
			 startFID, ActiveExpInfo.ExpInfo->AcqBaseBufName,  strsize);
	      ...
	      setStatGoFlag(ActiveExpInfo.ExpInfo->GoFlag);
              ...
              if (ActiveExpInfo.ExpInfo->ExpFlags & VPMODE_BIT)
                 strcpy(ExpN,"vp");
              else
	      ...
	      /* Send Vnmr Started Experiment */
              ...
              else if (ActiveExpInfo.ExpInfo->ExpFlags & VPMODE_BIT) 
              {
                  /* do nothing. Recvproc will queue the EXP_STARTED message */
              }
	...
	DPRINT(1,"chkExpQ: No Experiments in Queue.\n");
        setStatInQue(totalq);
        roboTerminate(); /* Terminate Roboproc */
     }
   } 
   else
   {
      ...
      * if (returnInterface == NULL) {
      *    errLogRet(ErrLogOp,debugInfo,"chkExpQ: No return interface\n" );
      * 	return( 0 );
      * }
      * strcpy(Msge,"\nwrite('line3','Connection to Console not Established Yet.')\n");
      * deliverMessage( returnInterface, Msge );
...
/* abort and turn off all processing */
int acqHalt2(char *args)
{
   ...
   send2Monitor(HALTACQ, 0, 0, 0, NULL, 0);
...
int acqAbort(char *args)
{
   ...
   send2Monitor(ABORTACQ, 0, 0, 0, NULL, 0);
...
int acqDebug(char *args)
{
   ...
   if (dlevel > 9)
   {
     ...
     /* writeChannel(chanId, &msge[ 0 ], CONSOLE_MSGE_SIZE); */
     send2Monitor(ACQDEBUG, dlevel, 0, 0, NULL, 0);
   }
   else
   {
...
int acqStop(char *args)
{
   ...
   if (isExpActive() != 0) 
   {
      ...
      if ( (isUserOk2Access(userName) != 0)  && (isExpNumOk2Access(expnum) != 0))
      {
        ...
        */
   	/* writeChannel(chanId, &msge[ 0 ], CONSOLE_MSGE_SIZE); */
        send2Monitor(STOP_ACQ, saMode, saMod, 0, NULL, 0);
...
int normalMode(char *args)
{
   ...
   /* No need to send message if Autoproc is not active */
...
/*   char	 msge[CONSOLE_MSGE_SIZE]; */
   ...
   if (cmdReturnStatus != NULL)
      roboStat = atoi(cmdReturnStatus);
   else
      roboStat = -1;  /* interperted as a timeout by the console */
   /*
   memset( &msge[ 0 ], 0, sizeof( msge ) );
   ...
   */
   /* writeChannel(chanId, &msge[ 0 ], CONSOLE_MSGE_SIZE); */
   send2Monitor(ROBO_CMD_ACK, roboStat, 0, 0, NULL, 0);
   return 0;
}
    ...
    /* Report back what kind of experiment is progress as well (GoFlag).  */
           if (ActiveExpInfo.ExpInfo->ExpFlags & AUTOMODE_BIT)
              sprintf(Msge,"%d auto %d\n", ActiveExpInfo.ExpInfo->ExpNum,
                  ActiveExpInfo.ExpInfo->GoFlag);
           else if ( ! strcmp(ActiveExpInfo.ExpInfo->UserName,"") )
              sprintf(Msge,"%d nobody %d\n", ActiveExpInfo.ExpInfo->ExpNum,
                  ActiveExpInfo.ExpInfo->GoFlag);
           else
              sprintf(Msge,"%d %s %d\n", ActiveExpInfo.ExpInfo->ExpNum,
                  ActiveExpInfo.ExpInfo->UserName,
...
{
   char     consoleMsg[ CONSOLE_MSGE_SIZE ];
   DPRINT( 1, "rebooted message\n" );
   if (consoleConn())
   {
      /* Set acqstate to Inactive */
      strcpy(consoleMsg,"2,46,0\n");
      send2Monitor(XPARSER, 0,0,0,consoleMsg, strlen(consoleMsg) );
      downloadShims();
   }
   return 0;
}
...
getStatBlock(char *args)
{
	...
	/* writeChannel( chanId, &consoleMsg[ 0 ], sizeof( consoleMsg ) ); */
        ...
        send2Monitor(GETSTATBLOCK, 0, 0, 0, NULL, 0);
...
/*  A series of tests and checks are required to prevent conflicts.  In all cases
    ...
    4)  If the automation daemon (Autoproc) is active, reply auto.		*/
		...
		// changed to diagnostic rather than error  3/23/2010
		// errLogRet( ErrLogOp, debugInfo,  
		DPRINT1( 1,
...
/*  The Roboproc is a special username which only the Roboproc is expected to
	...
	/* writeChannel(chanId, &console_msg[ 0 ], CONSOLE_MSGE_SIZE); */
        DPRINT1(1,"console_msg: '%s'\n",console_msg);

         /* format of string:
             1st - XPARSER (==2)
             2nd - number of tokens following (e.g. sethw('z1',125) number of tokes == 3)
             3rd 1st token (all tokes are numbers)
             4th 2nd token
             5th etc.

             sethw('z1',125):  2 3,13,2,256, 
             2 - XPARSER
             3 - 3 tokens
             13 - SHIMDAC const in acqcmds.h 
              2 - shimdac number
            256 - value to shimdac
 
            maximum arguments are 14 for SET_TUNE(i.e. 15-tokens), 6 for SHIMI
            most are 1 or 2 arguments.
        */
         

        /* Monitor_cmd.h  has cmd & arg1-5, plus string */
        /* int cmdMsgePub(int cmd, int arg1, int arg2, int arg3, 
         *                                         char *msgstr, long len)
         */
        if ( ! SystemReady4NextGo &&  ( ! strncmp(console_msg,"2,1,0,",6) ||
                                        ! strncmp(console_msg,"2,1,1,",6) ) )
        {
           DPRINT(1,"Don't send lockon or lockoff while exp. is active\n");
	   deliverMessage( returnInterface, "acquiring" );
        }
        else
        {
	   send2Monitor(XPARSER, 0,0,0,console_msg, console_len);
...
#ifdef INOVA_STYLE_JPSG_UPDATING
...
int call_jupdt(char *args)
{
   ...
   char         *cmd;
   char         *updtcmd;
   int 		cmdlen,updtcmdsize,trouble,bytes;
   authRecord	requester;
...
/* Send Recvproc its message first  */
	...
	memset( &msge[ 0 ], 0, sizeof( msge ) );
	sprintf(&msge[0],"%d%cACQI_PARSE,%s,%u,%u,%d;",
...
int isUserOk2Access(char* username)
{
   ...
   if ((int) strlen(ActiveExpInfo.ExpId) > 1) 
   {
	...
	if ( (strcmp(username,ActiveExpInfo.ExpInfo->UserName) != 0) &&
             ( strcmp("",ActiveExpInfo.ExpInfo->UserName)) )
	{
          stat = 0;
        }
     }
...
void rmPsgFiles(SHR_EXP_INFO ExpInfo)
{
   DPRINT3(1,"\n   rmPsgFiles: InitCodeFile: '%s', PreCodefile: '%s', PSCodefile: '%s'\n",
	ExpInfo->InitCodefile,ExpInfo->PreCodefile,ExpInfo->PSCodefile);
   DPRINT4(1,"\n   rmPsgFiles: PostCodefile: '%s', AcqRTTablefile: '%s', RTParmfile: '%s', WWaveFormFile: '%s'\n",
	ExpInfo->PostCodefile,ExpInfo->AcqRTTablefile,ExpInfo->RTParmFile,ExpInfo->WaveFormFile);
   ...
   if (ExpInfo->InteractiveFlag == 0)
   {
      ...
      if ((int) strlen(ExpInfo->InitCodefile) > 1)
      {
         ...
         strcpy(dirName, ExpInfo->InitCodefile);
         if (strlen(dirName) < 5)
            return;
         dirName[ strlen(dirName) - 5 ] = '\0';
         for (i=0; i < sizeof(cntrl) / sizeof(char *); i++ )
         {
            sprintf(fileName,"%s.init.%s", dirName, cntrl[i]);
            unlink(fileName);
            sprintf(fileName,"%s.ps.%s", dirName, cntrl[i]);
            unlink(fileName);
            sprintf(fileName,"%s.pat.%s", dirName, cntrl[i]);
            unlink(fileName);
...
int downloadDSP( char *args )
{
	...
	if (vmeAddrAscii == NULL) {
		...
		sprintf( vmeAddrAscii, "0x%x", confaddr->dspDownLoadAddr );
		mClose( md );
	}
...
void roboTerminate()
{
   ...
   if ( chkTaskActive(AUTOPROC) )
   {
      DPRINT(1, "Autoproc is active, just return\n");
      return;
   }
   DPRINT(1, "Expproc sending \'term\' to Roboproc\n");

   sendRoboCommand("term");
   return;
}
