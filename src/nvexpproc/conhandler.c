...
/*  Download shims at console bootup by first specifying the shim set,
    ...
    shims.  If the hardware does not support a particular shim, the
...
void downloadShims()
{
   ...
   val = getStatShimSet();
   if (val == -1)  // stat block has been cleared
      return;
   sprintf(downloadCmd,"2,%d,%d",SHIMSET,val);
   send2Monitor(XPARSER, 0,0,0,downloadCmd, strlen(downloadCmd) );
   shimIndex = 0;
   /* If MAX_SHIMS_CONFIGURED gets bigger than 48, some structures in
    * nvacq/shimHandler.c will need to be updated
    */
   while (shimIndex < MAX_SHIMS_CONFIGURED)
   {
   ...
   sprintf(downloadCmd,"11,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
              LKPOWER, getStatLkPower(), LKGAIN, getStatLkGain(),
              LKPHASE, getStatLkPhase(), LKMODE, (getStatLkMode()) ? 1 : 0,
              LKFREQ, getStatLockFreq1(), getStatLockFreq2() );
   send2Monitor(XPARSER, 0,0,0,downloadCmd, strlen(downloadCmd) );
   sprintf(downloadCmd,"8,%d,%d,%d,%d,%d,%d,%d,%d\n",
              VTINFO, getStatVTSet(), getStatVTC(),
              PNEUMATICS, getStatPneuBearing(), getStatPneuVtAir(),
                          getStatPneuVTAirLimits(), getStatPneuSpinner() );
   send2Monitor(XPARSER, 0,0,0,downloadCmd, strlen(downloadCmd) );
}
...
/**************************************************************
*
*  processChanMsge - Routine envoked to handler messages from the console
*
*   This Function is the Non-interrupt function called to handle the
*   Async Channel interrupt (SIGUSR2) as register via registerChannelAsync() call
*
*   called from  processMonitorCmd() nddscomfuncs.c which was registered to handle SIGUSR2
*
* RETURNS:
* void
*
		  ...
		  struct _hw_config	*conf;
		   ...
		   {  
		      ...
		      for ( index = 0; index < conf->STM_present; index++)
                      {
                         void *dspDwnLdAddr;
			 dspDwnLdAddr = conf->dspDownLoadAddrs[index];
                 ...
                 char *msgstr, unsigned long msglen )
{
   ...
   switch( donecode )
   {
     ...
     case EXP_CMPLT:
        DPRINT(0,"EXP_CMPLT should NEVER Happen!!!!!\n");

	break;

     case WARNING_MSG:	/* donecode */
        DPRINT1(1,"WARNING_MSG: %d\n",errorcode);
        if (errorcode > (WARNINGS+FIFOMISSING-1) && 
	    errorcode < (WARNINGS+AUTO_NOTBOOTED+1))
        {
	    wallMsge("Expproc: >>>>>  WARNING: %s \n", getAcqErrMsge(errorcode) );
        }
        else if (isExpActive() == 1)
        {
           /* Temporarily, report error as if it occurred on fid 1 */
           sendProcMsg(1,1,WARNING_MSG,errorcode);
	}
	break;
     ...
     case SOFT_ERROR:	/* donecode */
        ...
        {
           /* Temporarily, report error as if it occurred on fid 1 */
           sendProcMsg(1,1,SOFT_ERROR,errorcode);
	}
	break;
...
*/
	...
	break;
     ...
     case STOP_CMPLT:	/* donecode */
        ...
        if (isExpActive() == 1)
        {
          /* sendVnmrCp(STOP_CMPLT,doencode,0,0); */
	...
	DPRINT(1,"SYSTEM_READY\n");
        SystemReady4NextGo = 1;
        chkExpQ(" ");
#ifdef NOW_PERFORM_BY_ExpDone_VIA_expdone_msg_from_recvproc   /* 3/27/2006  GMB */
        if (isExpActive() == 1)
        {
	  resetState(); 
     ...
     case GET_SAMPLE:
        {
	     char robocmd[256]; 
             ...
             sprintf(robocmd,"getsmp %d",errorcode);
	     roboCmd(robocmd);
        }
		break;
     ...
     case REPUT_SAMPLE:
        {
	     char robocmd[256]; 
             ...
             DPRINT1(1,"REPUT_SAMPLE: %d\n",errorcode);
	     sprintf(robocmd,"reputsmp %d\n",errorcode);
	     roboCmd(robocmd);
        }
		break;
